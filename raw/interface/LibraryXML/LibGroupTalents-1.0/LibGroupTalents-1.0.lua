...
-- GetClassTalentData
-- Builds an internal table for talent name -> tree/index lookups using Ascension's C_CharacterAdvancement API.
function GetClassTalentData(unit)
	...
	if (class) then
		...
		if (not data) then
			...
			local specOrder = ClassInfoUtil.GetClassSpecOrder(class)
			if (specOrder) then
				for tab = 1, #specOrder do
                    local class = specOrder[tab][1]
                    local dbcClass = CharacterAdvancementUtil.GetClassDBCByFile(class)
					local specFile = specOrder[tab][2]
					local dbcSpec = CharacterAdvancementUtil.GetSpecDBCByFile(specFile)
					local tree = new()
					local classInfo = C_ClassInfo and C_ClassInfo.GetSpecInfo and C_ClassInfo.GetSpecInfo(class, specFile)
					tree.name = classInfo and classInfo.Name or specFile
					tree.icon = classInfo and classInfo.Icon or nil
					tree.background = ("default-talent-%s-%s"):format(class, specFile)
					tinsert(data, tree)

					tree.list = new()
					local entries = C_CharacterAdvancement.GetTalentsByClass(dbcClass, dbcSpec, true) or {}
					for i, entry in ipairs(entries) do
						local name, _, icon = GetSpellInfo(entry.Spells and entry.Spells[1])
						if (name) then
							local e = new()
							e.name = name
							e.icon = icon
							...
							e.maxRank = entry.Spells and #entry.Spells or 1
							e.index = i
							e.treeIndex = tab
							e.id = entry.ID
							tinsert(tree.list, e)
...
-- ReadTalentGroup
local function ReadTalentGroup(unit, group, class)
...
--@end-debug@]===]
	...
	if (ctd) then
		local n = new()
		for tab = 1, 3 do
			local branchLength = #ctd[tab].list
			local t = new()
			local trim
			for i = 1, branchLength do
				local info = ctd[tab].list[i]
				local currentRank = C_CharacterAdvancement.UnitTalentRankByID(unit, info.id, group) or 0
...
function lib:TalentQuery_Ready(e, name, realm, unit)
	...
	if (r) then
		local namerealm = realm and realm ~= "" and name .. "-" .. realm or name
		local active, unlocked = C_CharacterAdvancement.GetInspectInfo(unit)
		if (active) then
			local numActive = (unlocked and #unlocked) or 1
			local invalid
			local talents = new()

			for group = 1, numActive do
				-- In Ascension, UnitTalentRankByID expects a spec/build identifier, not a 1-based index.
				-- Use the value from 'unlocked' when available; otherwise fall back to the index.
				local specOrGroup = (unlocked and unlocked[group]) or group
				local n = ReadTalentGroup(unit, specOrGroup, r.class)
				if (n and #n >= 3) then
					talents[group] = n
				else
					invalid = true
			...
			if (not invalid) then
				...
				self:OnReceiveTalents(guid, unit, talents, active, numActive, nil)
			end
		end
	end
end
...
function lib:GetActiveTalentGroup(unit)
	if (UnitIsUnit(unit, "player")) then
		local active = C_CharacterAdvancement.GetInspectInfo(unit)
		return active or 1
	else
		local guid = unit and UnitGUID(unit)
		local r = guid and self.roster[guid]
...
function lib:GetNumTalentGroups(unit)
	if (UnitIsUnit(unit, "player")) then
		local _, unlocked = C_CharacterAdvancement.GetInspectInfo(unit)
		return (unlocked and #unlocked) or 1
	else
		local guid = unit and UnitGUID(unit)
		local r = guid and self.roster[guid]
	...
	if (UnitIsUnit(unit, "player")) then
		...
		if (class) then
			...
			if (ctd and tab >= 1 and tab <= #ctd) then
				local active = group or self:GetActiveTalentGroup(unit)
				local n = ReadTalentGroup(unit, active, class)
				local s = n and n[tab]
				local spent = 0
				if (s) then
					for i = 1, #s do
						spent = spent + ((s:byte(i) or 48) - 48)
					end
				end
				return ctd[tab].name, ctd[tab].icon, spent, ctd[tab].background, 0
			end
		end
	else
		local guid = unit and UnitGUID(unit)
	...
	if (UnitIsUnit(unit, "player")) then
		...
		if (class) then
			...
			if (ctd and tab >= 1 and tab <= #ctd) then
				...
				if (info) then
					local active = group or self:GetActiveTalentGroup(unit)
					local rank = C_CharacterAdvancement.UnitTalentRankByID(unit, info.id, active) or 0
					return info.name, info.icon, info.tier, info.column, rank, info.maxRank
				end
			end
		end
	else
...
function lib:GetUnspentTalentPoints(unit, group)
	if (UnitIsUnit(unit, "player")) then
		-- Not available via C_CharacterAdvancement; return nil
		return nil
	else
		local guid = unit and UnitGUID(unit)
		local r = guid and self.roster[guid]
