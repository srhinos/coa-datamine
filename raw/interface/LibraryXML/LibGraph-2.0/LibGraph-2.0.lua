...
local TextureDirectory = "Interface\\LibraryXML\\LibGraph-2.0\\"
...
function lib:DrawLine(C, sx, sy, ex, ey, w, color, layer, linetexture)
	...
	local lineFactor = TAXIROUTE_LINEFACTOR_2
	...
	if (dy >= 0) then
		boundingWidth = ((lineLength * cos) - (w * sin)) * lineFactor
		boundingHeight = ((w * cos) - (lineLength * sin)) * lineFactor

		bottomLeftX = (w / lineLength) * sinCos
		bottomLeftY = sin * sin
		bottomRightY = (lineLength / w) * sinCos
		...
		bottomLeftX = sin * sin
		bottomLeftY = -(lineLength / w) * sinCos
		bottomRightX = 1 + (w / lineLength) * sinCos
	...
	T:ClearAllPoints()
	T:SetTexCoord(topLeftX, topLeftY, bottomLeftX, bottomLeftY, topRightX, topRightY, bottomRightX, bottomRightY)
	T:SetPoint("BOTTOMLEFT", C, relPoint, cx - boundingWidth, cy - boundingHeight)
	T:SetPoint("TOPRIGHT", C, relPoint, cx + boundingWidth, cy + boundingHeight)
	T:Show()
	return T
end
...
function lib:TestGraph2Lib()
	DEFAULT_CHAT_FRAME:AddMessage("Testing "..major, "")
	TestRealtimeGraph()
	TestLineGraph()
	TestScatterPlot()
	TestPieChart()
