...
RAID_CLASS_BUTTONS = { };
--[[do
	RAID_CLASS_BUTTONS["PETS"]			= { button = 1, coords = {0, 1, 0, 1} };
	RAID_CLASS_BUTTONS["MAINTANK"]		= { button = 2, coords = {0, 1, 0, 1} };
	RAID_CLASS_BUTTONS["MAINASSIST"]	= { button = 3, coords = {0, 1, 0, 1} };
end]]
MAX_RAID_CLASS_BUTTONS = 0;
	...
	for i=1, MAX_RAID_MEMBERS do
		...
		if ( i <= numRaidMembers ) then
			...
			if ( raidGroup.nextIndex <= MEMBERS_PER_RAID_GROUP ) then
				...
				if RAID_SUBGROUP_LISTS[subgroup] then
					tinsert(RAID_SUBGROUP_LISTS[subgroup], i);
				end

				
				if ( fileName == "HUNTER" or fileName == "WARLOCK" ) and RAID_SUBGROUP_LISTS["PETS"] then
					tinsert(RAID_SUBGROUP_LISTS["PETS"], i);
				end
				
				-- Place Main Tank & Main Assist into a subgroup
				if ( role and RAID_SUBGROUP_LISTS[role] ) then
...
function RaidPulloutDropDown_OnLoad(self)
	self.raidPulloutDropDown = self:GetParent();
...
function RaidPulloutDropDown_Initialize()
	...
	local currentPullout = UIDROPDOWNMENU_OPEN_MENU.raidPulloutDropDown;
