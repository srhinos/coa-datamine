...
function AuctionFrame_OnShow (self)
	...
	UIDropDownMenu_SetSelectedValue(BrowseDropDown, 6)
...
function AuctionFrameBrowse_OnLoad(self)
	...
	-- set default sort
	AuctionFrame_SetSort("list", "buyout", false);
...
function AuctionFrameBrowse_UpdateArrows()
	...
	SortButton_UpdateArrow(BrowseCurrentBidSort, "list", "buyout");
end
...
local EnchantBorders = {
	...
	[0] = "Interface\\AddOns\\AwAddons\\Textures\\enchant\\EnchantBorder_white", -- Quality 0 = missing enchant
}
...
function AuctionFrameBrowse_Update()
	...
	for i=1, NUM_BROWSE_TO_DISPLAY do
		...
		if ( index > (numBatchAuctions + (NUM_AUCTION_ITEMS_PER_PAGE * AuctionFrameBrowse.page)) ) then
			...
			buyoutFrame = _G[buttonName.."BuyoutMoneyFrame"];
			if ( buyoutPrice > 0 ) then
				moneyFrame:SetPoint("RIGHT", button, "RIGHT", 10, 10);
				MoneyFrame_Update(buyoutFrame, buyoutPrice);
				buyoutFrame:Show();
			else
...
function AuctionFrameAuctions_Update()
	...
	for i=1, NUM_AUCTIONS_TO_DISPLAY do
		...
		if ( index > (numBatchAuctions + (NUM_AUCTION_ITEMS_PER_PAGE * AuctionFrameAuctions.page)) ) then
			...
			buttonBuyoutFrame = _G[buttonName.."BuyoutMoneyFrame"];
			if ( saleStatus == 1 ) then
				...
				if ( buyoutPrice > 0 ) then
					...
					MoneyFrame_Update(buttonBuyoutFrame, buyoutPrice);
