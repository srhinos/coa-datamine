...
function Postal_BlackBook:AddAlt()
	...
	self:AddAscensionAlt()
	table.sort(db)
	self:UnregisterEvent("PLAYER_ENTERING_WORLD")
	self.AddAlt = nil -- Kill ourselves so we only run it once
end

function Postal_BlackBook:AddAscensionAlt()
	for i = 1, C_AccountInfo.GetNumCharacters() do
		local character = C_AccountInfo.GetCharacterAtIndex(i)
		local realm = GetRealmName()
		local faction = character.Faction
		local player = character.CharacterName
		local level = character.Level
		local _, class = character.Class:sub(7)
		if realm and faction and player and level and class then
			local namestring = ("%s|%s|%s|%s|%s"):format(player, realm, faction, level, class)
			local db = Postal.db.global.BlackBook.alts
			enableAltsMenu = false
			enableAllAltsMenu = false
			for j = #db, 1, -1 do
				local p, r, f, l, c = strsplit("|", db[j])
				if p == player and r == realm and f == faction then
					tremove(db, j)
				end
				if p ~= player and r == realm and f == faction then
