...
function ClassTrainer_SetSelection(id)
	...
	local serviceDescription = GetTrainerServiceDescription(id)
	local description, newLines = C_Format.Format(serviceDescription, true)
	if description and newLines then
		if newLines then
			for _, line in ipairs(newLines) do
				description = description .. "\n" .. ColorUtil.WrapText(line, 1, 1, 1)
			end
		end
		ClassTrainerSkillDescription:SetText( description );
	else
		ClassTrainerSkillDescription:SetText( serviceDescription );
	end
