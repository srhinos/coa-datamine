...
COMBAT_TEXT_TYPE_INFO["DAMAGE_CRIT"] = {r = 1, g = 0.1, b = 0.1, var = "COMBAT_TEXT_SHOW_INCOMING_DAMAGE" };
COMBAT_TEXT_TYPE_INFO["DAMAGE"] = {r = 1, g = 0.1, b = 0.1, isStaggered = 1, var = "COMBAT_TEXT_SHOW_INCOMING_DAMAGE" };
COMBAT_TEXT_TYPE_INFO["OUTGOING_DAMAGE"] = {r = 1, g = 1, b = 0, isStaggered = 1, var = "COMBAT_TEXT_SHOW_OUTGOING_DAMAGE" };
COMBAT_TEXT_TYPE_INFO["OUTGOING_DAMAGE_CRIT"] = {r = 1, g = 1, b = 0, var = "COMBAT_TEXT_SHOW_OUTGOING_DAMAGE" };
...
COMBAT_TEXT_TYPE_INFO["SPELL_DAMAGE_CRIT"] = {r = 0.79, g = 0.3, b = 0.85, var = "COMBAT_TEXT_SHOW_INCOMING_DAMAGE" };
COMBAT_TEXT_TYPE_INFO["SPELL_DAMAGE"] = {r = 0.79, g = 0.3, b = 0.85, var = "COMBAT_TEXT_SHOW_INCOMING_DAMAGE" };
...
function CombatText_OnEvent(self, event, ...)
	...
	local scrollFunction = COMBAT_TEXT_SCROLL_FUNCTION;
	...
	elseif ( event == "UNIT_COMBAT" ) then
		...
		if data == "WOUND" then
			if arg3 == "CRITICAL" then
				messageType = "OUTGOING_DAMAGE_CRIT"
			else
				messageType = "OUTGOING_DAMAGE"
			end
		else
			return
		end
	...
	if ( messageType == "" ) then
	elseif messageType == "OUTGOING_DAMAGE" then
		message = arg4
		scrollFunction = CombatText_FountainScroll
	elseif messageType == "OUTGOING_DAMAGE_CRIT" then
		displayType = "crit"
		message = arg4
		scrollFunction = CombatText_FountainScroll
	...
	elseif ( messageType == "RESIST" or messageType == "SPELL_RESIST" ) then
		if ( arg3 ) then
			if arg3 < 0 then
				message = data.." "..format(VULNERABLE_TRAILER, -arg3)
			else
				-- Partial resist
	...
	-- Add the message
	if ( message ) then
		CombatText_AddMessage(message, scrollFunction, info.r, info.g, info.b, displayType, isStaggered);
	end	
end
...
function CombatText_OnUpdate(self, elapsed)
	...
	if ( (COMBAT_TEXT_Y_SCALE ~= UIParent:GetHeight() / 768) or (COMBAT_TEXT_X_SCALE ~= UIParent:GetWidth() / 1024) ) then
		CombatText_UpdateDisplayedMessages();
	end
end
...
function CombatText_UpdateDisplayedMessages()
	...
	-- Get scale
	COMBAT_TEXT_Y_SCALE = UIParent:GetHeight() / 768;
	COMBAT_TEXT_X_SCALE = UIParent:GetWidth() / 1024;
