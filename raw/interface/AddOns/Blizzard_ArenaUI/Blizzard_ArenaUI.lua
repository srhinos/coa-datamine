...
function ArenaEnemyFrame_UpdatePlayer(self, useCVars)--At some points, we need to use CVars instead of UVars even though UVars are faster.
	...
	if class == "HERO" then
		local primaryStat = UnitPrimaryStat(self.unit)
		if primaryStat then
			local spellID, _, icon = C_PrimaryStat:GetPrimaryStatInfo(primaryStat)
			self.classPortrait:SetPortraitTexture(icon);
		else
			self.classPortrait:SetPortraitTexture("Interface\\Icons\\classicon_"..class)
		end
	elseif class then
		self.classPortrait:SetPortraitTexture("Interface\\Icons\\classicon_"..class)
	end
