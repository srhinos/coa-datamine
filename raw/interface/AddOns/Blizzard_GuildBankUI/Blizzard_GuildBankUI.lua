...
function GuildBankFrame_OnLoad(self)
	...
	C_Hook:Register(GuildBankFrame, "BANK_PERMISSIONS_PAYLOAD", function()
		local isPersonalBank, isRealmBank = GetBankPermissions()
		if isPersonalBank ~= nil then
			self.IsPersonalBank = isPersonalBank
			self.IsRealmBank = isRealmBank
...
function GuildBankFrame_UpdateTabBuyingInfo()
	if GuildBankFrame.IsPersonalBank or GuildBankFrame.IsRealmBank then
		GuildBankFrameTabCost:Hide()
		GuildBankFrameTabCostMoneyFrame:Hide()
		GuildBankFramePurchaseButton:Hide()
		if GuildBankFrame.IsPersonalBank then
			GuildBankFrameBuyInfoText:SetText(PERSONAL_BANK_BUY_TEXT)
			GuildBankFrameBuyInfoNumTabsPurchasedText:SetText(PERSONAL_BANK_BUY_INFO_TEXT)
		else
			GuildBankFrameBuyInfoText:SetText(REALM_BANK_BUY_TEXT)
			GuildBankFrameBuyInfoNumTabsPurchasedText:SetText(REALM_BANK_BUY_INFO_TEXT)
		end
	else
		GuildBankFrameTabCost:Show()
		GuildBankFrameTabCostMoneyFrame:Show()
		GuildBankFramePurchaseButton:Show()
		GuildBankFrameBuyInfoText:Show()
		local tabCost = GetGuildBankTabCost();
		local numTabs = GetNumGuildBankTabs();
		GuildBankFrameBuyInfoText:SetText(PURCHASE_TAB_TEXT)
...
function GuildBankFrame_UpdateTabs()
	...
	for i=1, MAX_GUILDBANK_TABS do
		...
		if ( i == tabToBuyIndex and (IsGuildLeader() or GuildBankFrame.IsPersonalBank or GuildBankFrame.IsRealmBank) ) then
			iconTexture:SetTexture("Interface\\GuildBankFrame\\UI-GuildBankFrame-NewTab");
			if GuildBankFrame.IsPersonalBank then
				tabButton.tooltip = BUY_PERSONAL_BANK_TAB
			elseif GuildBankFrame.IsRealmBank then
				tabButton.tooltip = BUY_REALM_BANK_TAB
			else
			...
			if ( disableAll or GuildBankFrame.mode == "log" or GuildBankFrame.mode == "tabinfo" ) then
				...
				if ( i == currentTab ) then
					...
					if GuildBankFrame.IsPersonalBank then
						titleText = BUY_PERSONAL_BANK_TAB
					elseif GuildBankFrame.IsRealmBank then
						titleText = BUY_REALM_BANK_TAB
					else
	...
	if GuildBankFrame.IsPersonalBank or GuildBankFrame.IsRealmBank then
		...
		GuildBankMoneyFrame:Hide()
		GuildBankFrameWithdrawButton:Hide()
		GuildBankFrameDepositButton:Hide()
		GuildBankMoneyUnlimitedLabel:Hide()
		GuildBankMoneyLimitLabel:SetText(GuildBankFrame.IsPersonalBank and PERSONAL_BANK or REALM_BANK)
		GuildBankFrameTab1:SetText(GuildBankFrame.IsPersonalBank and PERSONAL_BANK or REALM_BANK)
	else
		GuildBankFrameTab2:Show()
		GuildBankFrameTab3:Show()
		GuildBankFrameTab4:Show()
		GuildBankMoneyFrame:Show()
		GuildBankFrameWithdrawButton:Show()
		GuildBankFrameDepositButton:Show()
		GuildBankMoneyLimitLabel:SetText(GUILDBANK_AVAILABLE_MONEY)
		GuildBankFrameTab1:SetText(GUILD_BANK)
	end

	PanelTemplates_TabResize(GuildBankFrameTab1, 0)
end
...
function GuildBankTab_OnClick(self, mouseButton, currentTab)
	...
	GuildBankFrame_UpdateTabs();
	if ( (IsGuildLeader() or GuildBankFrame.IsPersonalBank or GuildBankFrame.IsRealmBank) and mouseButton == "RightButton" and currentTab ~= (GetNumGuildBankTabs() + 1) ) then
