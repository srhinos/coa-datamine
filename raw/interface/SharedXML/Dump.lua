...
local function GetScrollingMessageFrame()
    return DEFAULT_CHAT_FRAME or DevConsole
end
...
local function DevTools_WriteConsole(self, msg)
    DevConsole:AddMessage(msg);
end
...
function DevTools_DumpConsole(value, startKey)
    ...
    context.GetUserdataName = Pick_Cache_Function(DevTools_Cache_Userdata,
            DEVTOOLS_USE_USERDATA_CACHE);
    context.Write = DevTools_WriteConsole;
...
function DevTools_DumpConsoleCommand(msg, editBox)
    if not DevConsole then
        if InGlue() then
            return
        else
    ...
    if (string_match(msg,"^[A-Za-z_][A-Za-z0-9_]*$")) then
        ...
        if (val == nil) then
            local key = string_format(FORMATS.tableKeyAssignPrefix,
                    '', prepSimpleKey(msg, {}));
            DevConsole:Print(key .. "nil,");
        else
            tmp[msg] = val;
        end
        DevTools_DumpConsole(tmp);
        return;
    end

    local func,err = loadstring("return " .. msg);
    if (not func) then
        DevConsole:Print("Dump: |cffff0000ERROR:|r " .. err);
    else
        DevTools_DumpConsole({ func() }, "value");
    end
end
