...
function BlizzardOptionsPanel_Slider_Disable (slider)
	...
	_G[name.."Value"]:SetVertexColor(GRAY_FONT_COLOR.r, GRAY_FONT_COLOR.g, GRAY_FONT_COLOR.b);
end
...
function BlizzardOptionsPanel_Slider_Enable (slider)
	...
	_G[name.."Value"]:SetVertexColor(NORMAL_FONT_COLOR.r, NORMAL_FONT_COLOR.g, NORMAL_FONT_COLOR.b);
end
...
function BlizzardOptionsPanel_Slider_Refresh (slider)
	...
	if ( value ) then
		...
		slider.value = value;
		if slider.valueTextFunc then
			_G[slider:GetName().."Value"]:SetText(slider.valueTextFunc(slider, value))
		elseif slider.displayAsPercent then
			_G[slider:GetName().."Value"]:SetText(MTrunc(value * 100, slider.displayPrecision or 0).."%")
		else
			_G[slider:GetName().."Value"]:SetText(MTrunc(value, slider.displayPrecision or 2))
		end
	end
end
...
function BlizzardOptionsPanel_Slider_OnValueChanged (self, value)
	...
	if self.valueTextFunc then
		_G[self:GetName().."Value"]:SetText(self.valueTextFunc(self, value))
	elseif self.displayAsPercent then
		_G[self:GetName().."Value"]:SetText(MTrunc(value * 100, self.displayPrecision or 0).."%")
	else
		_G[self:GetName().."Value"]:SetText(MTrunc(value, self.displayPrecision or 2))
	end
end
...
function BlizzardOptionsPanel_CheckButton_OnClick (checkButton)
	...
	if ( checkButton.setFunc ) then
		checkButton.setFunc(setting, checkButton);
	end
...
function BlizzardOptionsPanel_CheckButton_Refresh (checkButton)
	...
	elseif ( checkButton.bitCVar ) then
		value = C_CVar.GetBitfield(checkButton.bitCVar, checkButton.bit) and "1" or "0"
	...
	if ( value ) then
		...
		if checkButton.invertedDependentControls then
			if ( checkButton:GetChecked() ) then
				for _, depControl in SecureNext, checkButton.invertedDependentControls do
					depControl:Disable();
				end
			else
				for _, depControl in SecureNext, checkButton.invertedDependentControls do
...
function BlizzardOptionsPanel_OnEvent (frame, event, ...)
	if ( event == "PLAYER_ENTERING_WORLD" or event == "FRAMES_LOADED" ) then
			...
			for i, control in SecureNext, frame.controls do
				...
				if ( entry ) then
					if ( entry.text ) then
						...
						control.disableTooltipText = (_G["OPTION_DISABLE_TOOLTIP_" .. gsub(entry.text, "_TEXT$", "")] or entry.disableTooltip);
					...
					elseif ( control.bitCVar ) then
						control.defaultValue = C_CVar.GetBitfield(control.bitCVar, control.bit) and "1" or "0"
					else
...
function BlizzardOptionsPanel_SetupControl (control)
	if ( control.type == CONTROLTYPE_CHECKBOX ) then
		...
		elseif ( control.bitCVar ) then
			local value = C_CVar.GetBitfield(control.bitCVar, control.bit) and "1" or "0";
			control.value = value;

			if ( control.uvar ) then
				_G[control.uvar] = value;
			end
			control.GetValue = function(self) return C_CVar.GetBitfield(self.bitCVar, self.bit) and "1" or "0" end
			control.SetValue = function(self, value) self.value = value; C_CVar.SetBitfield(self.bitCVar, self.bit, value == "1") end
			control.Disable = function (self) getmetatable(self).__index.Disable(self) _G[self:GetName().."Text"]:SetTextColor(GRAY_FONT_COLOR.r, GRAY_FONT_COLOR.g, GRAY_FONT_COLOR.b) end;
			control.Enable = function (self)
				getmetatable(self).__index.Enable(self);
				local text = _G[self:GetName().."Text"];
	...
	elseif ( control.type == CONTROLTYPE_SLIDER ) then
		...
		control.value = value;
		if control.valueTextFunc then
			_G[control:GetName().."Value"]:SetText(control.valueTextFunc(control, value))
		elseif control.displayAsPercent then
			_G[control:GetName().."Value"]:SetText(MTrunc(value * 100, control.displayPrecision or 0).."%")
		else
			_G[control:GetName().."Value"]:SetText(MTrunc(value, control.displayPrecision or 2))
		end
...
function BlizzardOptionsPanel_SetupInverseDependentControl (dependency, control)
	if ( not dependency ) then
		return;
	end
	...
	dependency.invertedDependentControls = dependency.invertedDependentControls or {};
	tinsert(dependency.invertedDependentControls, control);
