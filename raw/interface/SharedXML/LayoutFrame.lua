...
function BaseLayoutMixin:OnLoad()
    AttributesToKeyValues(self, "ignoreInLayout", "layoutIndex", "expand", "align", "defaultAlign",
            "leftPadding", "rightPadding", "topPadding", "bottomPadding",
            "fixedWidth", "fixedHeight",
            "minimumWidth", "maximumWidth", "minimumHeight", "maximumHeight",
            "spacing", "respectChildScale")
end
...
function IsLayoutFrame(f)
    return f.IsLayoutFrame and f:IsLayoutFrame() or false
end

local function InitializeLayoutChild(f)
    if not f.layoutInitialized then
        AttributesToKeyValues(f, "layoutHidden", "ignoreInLayout", "layoutIndex", "expand", "align",
                "leftPadding", "rightPadding", "topPadding", "bottomPadding")
        f.layoutInitialized = true
    end
end
...
function BaseLayoutMixin:AddLayoutChildren(layoutChildren, ...)
    for i = 1, select("#", ...) do
        ...
        InitializeLayoutChild(region)
        if (region:IsShown() or (region.layoutHidden and not region:GetAttribute("forcehidden"))) and not region.ignoreInLayout and (self:IgnoreLayoutIndex() or region.layoutIndex) then
...
function LayoutIndexComparator(left, right)
    if (left.layoutIndex == right.layoutIndex and left ~= right) then
        ...
        dprint(("Duplicate layoutIndex found: %d for %s and %s"):format(left.layoutIndex, leftName, rightName))
    end
...
VerticalLayoutMixin = CreateFromMixins("LayoutMixin")
...
function VerticalLayoutMixin:LayoutChildren(children, expandToWidth)
    ...
    for i, child in ipairs(children) do
        ...
        local childScale = child.GetScale and child:GetScale() or 1
        ...
        -- Set child position
        align = child.align or self.defaultAlign
...
IgnoreLayoutIndexVerticalLayoutMixin = CreateFromMixins("VerticalLayoutMixin")

function IgnoreLayoutIndexVerticalLayoutMixin:IgnoreLayoutIndex()
    return true
end
...
IgnoreLayoutIndexHorizontalLayoutMixin = CreateFromMixins("HorizontalLayoutMixin")

function IgnoreLayoutIndexHorizontalLayoutMixin:IgnoreLayoutIndex()
    return true
end
...
local function GetExtents(childFrame, left, right, top, bottom, layoutFrameScale)
    ...
    frameTop = frameTop + (childFrame.topPadding or 0)
    frameBottom = frameBottom - (childFrame.bottomPadding or 0)
    frameLeft = frameLeft - (childFrame.leftPadding or 0)
    frameRight = frameRight + (childFrame.rightPadding or 0)
    
    frameWidth = frameWidth + (childFrame.leftPadding or 0) + (childFrame.rightPadding or 0)
    frameHeight = frameHeight + (childFrame.topPadding or 0) + (childFrame.bottomPadding or 0)
...
function ResizeLayoutMixin:OnLoad()
    ...
    AttributesToKeyValues(self, "fixedWidth", "fixedHeight",
...
function ResizeLayoutMixin:Layout()
    ...
    local layoutFrameScale = self:GetScale()
    for childIndex, child in ipairs(self:GetLayoutChildren()) do
        ...
        if self.debugLayout then
            dprint("ResizeLayoutFrame", self:GetDebugName(), "Child", child:GetDebugName(), "Extents:", l, ",", r, ",", t, ",", b, "Defaulted", d)
        end
    ...
    if self.debugLayout then
        dprint("ResizeLayoutFrame", self:GetDebugName(), "Extents:", left, ",", right, ",", top, ",", bottom, "Defaulted", defaulted)
    end
    ...
    if left and right and top and bottom then
        ...
        local width = GetSize(self:GetWidthPadding(), self:GetFixedWidth(), self.minimumWidth, self.maximumWidth)
        local height = GetSize(self:GetHeightPadding(), self:GetFixedHeight(), self.minimumHeight, self.maximumHeight)
        self:SetSize(width, height)
    end
...
GridLayoutFrameMixin = CreateFromMixins("ResizeLayoutMixin")

function GridLayoutFrameMixin:OnLoad()
    ResizeLayoutMixin.OnLoad(self)
    AttributesToKeyValues(self, "childXPadding", "childYPadding", "isHorizontal", "stride",
            "layoutFramesGoingRight", "layoutFramesGoingUp")
end
...
function GridLayoutFrameMixin:ShouldUpdateLayout(layoutChildren)
    ...
    if #self.oldGridSettings.layoutChildren ~= #layoutChildren
            ...
            or self.oldGridSettings.layoutFramesGoingUp ~= self.layoutFramesGoingUp
            or self.oldGridSettings.minimumWidth ~= self.minimumWidth
            or self.oldGridSettings.maximumWidth ~= self.maximumWidth
            or self.oldGridSettings.minimumHeight ~= self.minimumHeight
            or self.oldGridSettings.maximumHeight ~= self.maximumHeight then
        return true
    end
