...
FrameUtil.RegisterFrameForUnitEvents = FrameUtil.RegisterFrameForEvents

function FrameUtil.IsDialogStyleGlobalMouseDown(frame, ...)
...
function FrameUtil.DialogStyleGlobalMouseDown(frame, ...)
    if FrameUtil.IsDialogStyleGlobalMouseDown(frame, ...) then
        frame:Hide()
    end
end
...
function FrameUtil.SetRegionsShown(regions, shown)
    for _, region in ipairs(regions) do
        region:SetShown(shown)
    end
end

function FrameUtil.SetRegionsSize(regions, width, height)
    for _, region in ipairs(regions) do
        region:SetSize(width, height)
    end
end

function FrameUtil.SetRegionsWidth(regions, width)
    for _, region in ipairs(regions) do
        region:SetWidth(width)
    end
end

function FrameUtil.SetRegionsHeight(regions, height)
    for _, region in ipairs(regions) do
        region:SetHeight(height)
    end
end

function FrameUtil.PassScriptToParent(self, script)
    local parent = self:GetParent()
    local func = parent and parent:GetScript(script)
    if func then
        func(parent)
    end
end

function FrameUtil.PassOnEnterToParent(self)
    FrameUtil.PassScriptToParent(self, "OnEnter")
end

function FrameUtil.PassOnLeaveToParent(self)
    FrameUtil.PassScriptToParent(self, "OnLeave")
end

function FrameUtil.PassOnClickToParent(self)
    FrameUtil.PassScriptToParent(self, "OnClick")
end

function FrameUtil.PassOnDragStartToParent(self)
    FrameUtil.PassScriptToParent(self, "OnDragStart")
end

function FrameUtil.PassOnDragStopToParent(self)
    FrameUtil.PassScriptToParent(self, "OnDragStop")
end
...
function GetUnscaledFrameRect(frame, scale)
    ...
    return (frameLeft * frameScale) / scale, (frameBottom * frameScale) / scale, (frameWidth * frameScale) / scale, (frameHeight * frameScale) / scale
end
...
function UIFrameFlash_OnUpdate(self, elapsed)
    ...
    while FLASHFRAMES[index] do
        ...
        if (frame.flashTimer == nil or frame.flashDuration == nil) or ( (frame.flashTimer > frame.flashDuration) and frame.flashDuration ~= -1 ) then
