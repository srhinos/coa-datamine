...
function GridLayoutUtil.GetGridPoint(index, parent, cellWidth, cellHeight, numColumns, xOffset, yOffset)
    return "TOPLEFT", parent, "TOPLEFT", xOffset + cellWidth * ((index - 1) % numColumns), yOffset - cellHeight * math.floor((index - 1) / numColumns)
end
GetGridPoint = GridLayoutUtil.GetGridPoint -- compat

function GridLayoutUtil.SimpleGridXY(column, row, cellWidth, cellHeight, xOffset, yOffset, xPadding, yPadding)
    return xOffset + ((cellWidth + xPadding) * column - 1), yOffset - ((cellHeight + yPadding) * row - 1)
end
