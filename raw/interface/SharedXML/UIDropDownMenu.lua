...
function UIDropDownMenu2_OnLoad(self)
	self.Left:SetAtlas("auctionhouse-ui-dropdown-left", Const.TextureKit.UseAtlasSize)
	self.Middle:SetAtlas("auctionhouse-ui-dropdown-middle", Const.TextureKit.IgnoreAtlasSize)
	self.Right:SetAtlas("auctionhouse-ui-dropdown-right", Const.TextureKit.UseAtlasSize)
	self.useUpdatedLayout = true
end
...
function UIDropDownMenuDelegate_OnAttributeChanged (self, attribute, value)
	...
	elseif ( attribute == "menuvalue" ) then
		UIDROPDOWNMENU_MENU_VALUE = value;
	elseif ( attribute == "initbutton" ) then
		local listFrameName = self:GetAttribute("initbutton-list")
		local index = self:GetAttribute("initbutton-index")
		if listFrameName and index then
...
info.icon = [nil, STRING] -- path to an icon for the button, right side
info.tCoordLeft, info.tCoordRight, info.tCoordTop, info.tCoordBottom = [nil, NUMBER, NUMBER, NUMBER, NUMBER] -- coords for the icon
info.iconAtlas = [nil, STRING] -- atlas for the icon, right side. Priority over info.icon
]]
...
function UIDropDownMenu_CreateFrames(level, index)
	...
	while ( index > UIDROPDOWNMENU_MAXBUTTONS ) do
		...
		for i=1, UIDROPDOWNMENU_MAXLEVELS do
			...
			newButton:SetFrameLevel(_G["DropDownList"..i]:GetFrameLevel() + 2)
		end
	end
end

function UIDropDownMenu_AddSpace(level)
	local spaceInfo = UIDropDownMenu_CreateInfo()
	spaceInfo.hasArrow = false
	spaceInfo.isTitle = true
	spaceInfo.notCheckable = true
...
function UIDropDownMenu_AddButton(info, level)
	...
	local button = _G[listFrameName.."Button"..index];
	UIDropDownMenuDelegate:SetAttribute("initbutton-list", listFrameName);
	UIDropDownMenuDelegate:SetAttribute("initbutton-index", index);
	UIDropDownMenuDelegate:SetAttribute("initbutton", true);
	...
	if ( info.hasArrow ) then
		...
		if info.icon or info.iconAtlas then
			icon:SetPoint("RIGHT", -10, 0)
			_G[listFrameName.."Button"..index.."ExpandArrow"]:SetPoint("RIGHT", 6, 0);
		else
			icon:SetPoint("RIGHT", 0, 0)
			_G[listFrameName.."Button"..index.."ExpandArrow"]:SetPoint("RIGHT", 0, 0);
		end
	else
		icon:SetPoint("RIGHT", 0, 0)
	...
	local checkButton = _G[listFrameName.."Button"..index.."Check"]

	if info.disabled or (info.isRadio and not checked) then
		checkButton:SetDesaturated(true)
		checkButton:SetAlpha(0.25)
	else
		checkButton:SetDesaturated(false)
		checkButton:SetAlpha(1)
	end
	
	button.isRadio = info.isRadio

	if info.isRadio then
		checkButton:SetTexture("Interface\\Buttons\\UI-RadioButton");
		checkButton:Show()

		if checked then
			button:LockHighlight();
			checkButton:SetTexCoord(0.25, 0.5, 0, 1)
		else
			button:UnlockHighlight();
			checkButton:SetTexCoord(0, 0.25, 0, 1)
		end
	else
		checkButton:SetTexture("Interface\\Buttons\\UI-CheckBox-Check")
		checkButton:SetTexCoord(0, 1, 0, 1)
...
function UIDropDownMenu_RefreshChecks()
	for i = 1, UIDROPDOWNMENU_MENU_LEVEL do
		local numButtons = _G["DropDownList"..i].numButtons
		for j = 1, numButtons do
			local button = _G["DropDownList"..i.."Button"..j]
			local checked = button.checked or false
			if type(button.checked) == "function" then
				checked = button.checked()
			end
			
			local checkButton = _G["DropDownList"..i.."Button"..j.."Check"]

			if button:IsEnabled() ~= 1 or (button.isRadio and not checked) then
				checkButton:SetDesaturated(true)
				checkButton:SetAlpha(0.25)
			else
				checkButton:SetDesaturated(false)
			...
			if button.isRadio then
				...
				if checked then
					button:LockHighlight();
					checkButton:SetTexCoord(0.26, 0.49, 0, 1)
				else
					button:UnlockHighlight();
					checkButton:SetTexCoord(0, 0.24, 0, 1)
				end
			else
				checkButton:SetTexture("Interface\\Buttons\\UI-CheckBox-Check")
				checkButton:SetTexCoord(0, 1, 0, 1)
...
function ToggleDropDownMenu(level, value, dropDownFrame, anchorName, xOffset, yOffset, menuList, button)
	...
	UIDropDownMenuDelegate:SetAttribute("menuvalue", value)
	local listFrame = _G["DropDownList"..level];
	...
	if ( listFrame:IsShown() and (UIDROPDOWNMENU_OPEN_MENU == tempFrame) ) then
		...
		if UIParent then
			...
			uiScale = 0.8 -- glue screen scale it way too big
		end
...
function UIDropDownMenu_JustifyText(frame, justification)
	...
	text:ClearAllPoints();
	if frame.useUpdatedLayout then
		text:SetPoint("LEFT", frame:GetName().."Left", "LEFT", 8, 2);
		text:SetPoint("RIGHT", frame:GetName().."Right", "RIGHT", -34, 2);
		text:SetJustifyH(justification);
	else
