...
MAX_PIN_LENGTH = 10;
--46.105.190.248 
SERVER_ALERT_URL = "http://api.ascension.gg/api/article/twitter"
SERVER_ALERT_BETA_URL = "http://api.ascension.gg/api/article/twitter"
SERVER_ALERT_PTR_URL = "http://api.ascension.gg/api/article/twitter"
...
function AccountLogin_OnShow(self)
	--[[local spellDBCDate, spellDBCTime, uiVersion, versionUpToDate = GetClientVersion()
	versionUpToDate = versionUpToDate and YES or NO
	AccountLoginVersion:SetFormattedText("DBC: %s@%s\nVersion: %s\nUp To Date: %s", spellDBCDate, spellDBCTime, uiVersion, versionUpToDate)]]
	CONNECTED_SAFE_CHECK = false
	
	local accountName, password, realmList = GetLastAccount()
	if IsGMClient and realmList then
		GLOBAL_REALMLIST = realmList
		SetCVar("realmList", GLOBAL_REALMLIST)
	end

	local shouldAutoLogin = false
	...
	if accountName ~= "" then
		...
		if (password == "") or (password == nil)  then
			...
			if not HAS_AUTO_LOGGED_IN then
				HAS_AUTO_LOGGED_IN = true
				shouldAutoLogin = IsAutoLogin and IsAutoLogin()
			end
		end
	else
	...
	if SetupRealmSelectDDL then
		SetupRealmSelectDDL()
	else 
		AccountLoginPasswordEdit:SetPoint("BOTTOM", 0, 259);
		AccountLoginLoginButton:SetPoint("BOTTOM", 3, 122);
...
function AccountLogin_OnEvent(event, arg1, arg2, arg3)
	if ( event == "SHOW_SERVER_ALERT" ) then
		-- twitter api doesnt work for this anymore, just hide
...
function AccountLogin_Login()
	...
	if ( AccountLoginSaveAccountName:GetChecked() ) then
		if ( AccountLoginSavePassword:GetChecked() ) then
			SaveAccount(AccountLoginAccountEdit:GetText(), AccountLoginPasswordEdit:GetText());
		else
			SaveAccount(AccountLoginAccountEdit:GetText());
		end
	else
		SaveAccount();
		SetUsesToken(false);
...
function AccountLogin_ShowUserAgreements()
	...
	ScanningText:Hide();
	if (not IsExtensionsLoaded() ) then
		AccountLoginUI:Hide();
		AccountLoginDropDown:Hide()
		GlueExtensionError:Show();
		return;
	elseif ( not EULAAccepted() ) then
...
function WoWAccountSelect_OnEvent(self, event)
	if ( event == "GAME_ACCOUNTS_UPDATED" ) then
		...
		for i = 1, GetNumGameAccounts() do
			...
			if ( name == UIDropDownMenu_GetText(AccountLoginDropDown) ) then
				selectedName = name;
				selectedIndex = i;
			end
			str = str .. name .. "|";
...
function WoWAccountSelect_Update()
	...
	FauxScrollFrame_Update(WoWAccountSelectDialogBackgroundContainerScrollFrame, count, MAX_ACCOUNTS_DISPLAYED, ACCOUNTNAME_BUTTON_HEIGHT);
end
...
function AccountLoginDropDown_OnClick(self)
	if self.value and self.value ~= "1" and self.value ~= "" then
		UIDropDownMenu_SetSelectedValue(AccountLoginDropDown, self.value);
		if IsGMClient then
			GLOBAL_REALMLIST = self.value
		end
...
function AccountLoginDropDown_Initialize()
	...
	for i = 1, #RealmChoices do
		RealmChoices[i].checked = (RealmChoices[i].text == selectedValue);
		UIDropDownMenu_AddButton(RealmChoices[i]);
	end
end
