...
function PetPaperDollFrame_Update()
	...
	local canGainXP = PetCanBeAbandoned()
	...
	elseif UnitLevel("pet") then
		PetLevelText:SetFormattedText(UNIT_TYPE_LEVEL_TEMPLATE, UnitLevel("pet"), "")
	else
...
-- Auto set new pets to defensive
C_Hook:Register("TamePet", "ASC_MISC", function(event, channel, sender)
	if sender ~= C_Player:GetName() then return end
	if event == "TAME_PET_SUCCESS" then
		CastPetAction(9) -- Defensive

		for i = 4, 7 do
			local autoCastAllowed, autoCastEnabled = select(6, GetPetActionInfo(i))
		...
		if C_Player:IsHero() and GetNumTalents(1, false, true) > 0 and unspent and unspent > 0 then
