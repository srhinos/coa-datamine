...
function PVPBattleground_UpdateBattlegrounds()
	...
	for i=1,GetNumBattlegroundTypes() do
		...
		localizedName, canEnter, isHoliday, isRandom, battlegroundID = GetBattlegroundInfo(i);
		-- @robinsch: Hack, I don't know where to make isRandom = true or Random Brawl queue
		local isRandomBrawl = battlegroundID == 33
		tempString = localizedName;
		if ( isRandom or isRandomBrawl or (canEnter and isHoliday) ) then
			if ( frame ) then
				frame.BGindex = i;
