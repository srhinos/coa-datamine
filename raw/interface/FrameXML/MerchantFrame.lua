...
function MerchantFrame_OnShow()
	...
	MerchantFrame.hasSoldJunk = false
	-- Update repair all button status
	MerchantFrame_UpdateCanRepairAll();
	MerchantFrame_UpdateGuildBankRepair();
	MerchantFrame_CheckAutoRepair();
...
function MerchantFrame_Update()
	if ( MerchantFrame.selectedTab == 1 ) then
		if C_CVar.GetBool("autoselljunk") and not MerchantFrame.hasSoldJunk then
			MerchantFrame_AutoSellJunkItems()
		end
...
function MerchantFrame_UpdateMerchantInfo()
	...
	for i=1, MERCHANT_ITEMS_PER_PAGE, 1 do
		...
		if ( index <= numMerchantItems ) then
			...
			local rating, teamSize1, teamSize2 = C_Item:GetMerchantItemRatingRequirement(index)

			if rating then
				local hasRating = false
				if teamSize1 or teamSize2 then
					if teamSize1 then
						local _, _, teamRating = GetArenaTeam(teamSize1)
						if teamRating >= rating then
							hasRating = true
						end
					end
					if teamSize2 and not hasRating then
						local _, _, teamRating = GetArenaTeam(teamSize2)
						if teamRating >= rating then
							hasRating = true
						end
					end
					...
					for j = 1, 3 do
						local _, _, teamRating = GetArenaTeam(j)
						if teamRating >= rating then
							hasRating = true
							break
						end
	...
	MerchantFrameSellJunkFrame:Show()
...
function MerchantFrame_UpdateAltCurrency(index, i)
    ...
    if honorPoints == -1 or arenaPoints == -1 or itemCount == -1 then
        SendSystemMessage(RED_FONT_COLOR:WrapText(MERCHANT_ALT_COST_MISSING))
        local name = GetMerchantItemInfo(index)
        dprint("Item", name, "at", index, "has an extended cost but is not found in the correct dbc.")
    end
    ...
    if itemCount == -1 then
        for i=1, MAX_ITEM_COST, 1 do
            ...
            itemTexture, itemValue, button.itemLink = "Interface\\Icons\\inv_misc_questionmark"
            itemValue = 0
            button.itemLink = "item:0"
...
function MerchantFrame_UpdateBuybackInfo()
	...
	MerchantRepairSettingsButton:Hide()
...
function MerchantFrame_UpdateRepairButtons()
	if ( CanMerchantRepair() ) then
		...
		if ( CanGuildBankRepair() ) then
			...
			MerchantRepairAllButton:SetPoint("BOTTOMRIGHT", MerchantFrame, "BOTTOMLEFT", 100, 89);
			MerchantRepairText:ClearAllPoints();
			MerchantRepairText:SetPoint("CENTER", MerchantFrame, "BOTTOMLEFT", 82, 129);
			...
			MerchantRepairAllButton:SetPoint("BOTTOMRIGHT", MerchantFrame, "BOTTOMLEFT", 157, 91);
			MerchantRepairText:ClearAllPoints();
			MerchantRepairText:SetPoint("BOTTOMLEFT", MerchantFrame, "BOTTOMLEFT", 11, 103);
			MerchantGuildBankRepairButton:Hide();
		end
		MerchantRepairText:Show();
		MerchantRepairAllButton:Show();
		MerchantRepairSettingsButton:Show()
...
function MerchantFrame_AutoSellJunkItems()
		...
		for bag = 0, 4 do
			for slot = 1, GetContainerNumSlots(bag) do
				...
				if itemID then
					...
					if quality and quality == 0 and invType and invType ~= "Quest" and vendorSell and vendorSell > 0 then
						local itemCount = select(2, GetContainerItemInfo(bag, slot)) or 1
						vendorSell = vendorSell * itemCount
						tinsert(junkToSell, {bag = bag, slot = slot, link = link, count = itemCount, vendorSell = vendorSell})
					end
				end
			end
		end
...
function MerchantFrame_CheckAutoRepair()
	if not CanMerchantRepair() then return end
	local repairSetting = C_CVar.GetNumber("autoRepair")
	if repairSetting > 0 then
		local repairAllCost, canRepair = GetRepairAllCost();
		if not canRepair then return end

		if repairSetting == 1 then
			RepairAllItems()
			SendSystemMessage(format(MERCHANT_AUTO_REPAIR_OWN_GOLD_SUCESSS, GetMoneyString(repairAllCost)))
		elseif repairSetting == 2 or repairSetting == 3 then
			-- repair via guild bank or guild bank + own gold
			...
			if amount > 0 and CanGuildBankRepair() then
				RepairAllItems(1)
				-- repair with own gold if guild bank can't afford it and we have both enabled
				if repairSetting == 3 and repairAllCost > amount and (repairAllCost - amount) <= GetMoney() then
					RepairAllItems()
					SendSystemMessage(format(MERCHANT_AUTO_REPAIR_BOTH_SUCCESS, GetMoneyString(amount), GetMoneyString(repairAllCost - amount)))
				else
					local diff = math.max(repairAllCost - amount, 0)
					SendSystemMessage(format(MERCHANT_AUTO_REPAIR_GUILD_BANK_SUCCESS, GetMoneyString(repairAllCost-diff)))
				end
			elseif repairSetting == 3 and canRepair then
				-- auto repair using own gold, we cant guild bank repair
				RepairAllItems()
				SendSystemMessage(format(MERCHANT_AUTO_REPAIR_OWN_GOLD_SUCESSS, GetMoneyString(repairAllCost)))
			end
		end
...
local function SelectRepairOption(button)
	C_CVar.Set("autoRepair", button.value)
	MerchantFrame_CheckAutoRepair()
	CloseDropDownMenus()
end

function MerchantRepairSettings_DropdownInitialize()
	local info
	local repairSetting = C_CVar.GetNumber("autoRepair")
	
	info = UIDropDownMenu_CreateInfo()
	info.text = MERCHANT_AUTO_REPAIR_SETTING
	info.isTitle = true
	...
	info.tooltipText = MERCHANT_AUTO_REPAIR_NONE_TOOLTIP
	info.tooltipOnButton = true
	info.checked = repairSetting == info.value
	info.func = SelectRepairOption
	UIDropDownMenu_AddButton(info)

	info = UIDropDownMenu_CreateInfo()
	info.text = MERCHANT_AUTO_REPAIR_USE_GOLD
	info.value = 1
	info.tooltipTitle = MERCHANT_AUTO_REPAIR_USE_GOLD
	info.tooltipText = MERCHANT_AUTO_REPAIR_USE_GOLD_TOOLTIP
	info.tooltipOnButton = true
	info.checked = repairSetting == info.value
	info.func = SelectRepairOption
	UIDropDownMenu_AddButton(info)
	...
	info = UIDropDownMenu_CreateInfo()
	info.text = MERCHANT_AUTO_REPAIR_USE_GUILD_BANK
	info.value = 2
	info.tooltipTitle = MERCHANT_AUTO_REPAIR_USE_GUILD_BANK
	info.tooltipText = MERCHANT_AUTO_REPAIR_USE_GUILD_BANK_TOOLTIP
	info.tooltipOnButton = true
	info.checked = repairSetting == info.value
	info.func = SelectRepairOption
	UIDropDownMenu_AddButton(info)
	...
	info = UIDropDownMenu_CreateInfo()
	info.text = MERCHANT_AUTO_REPAIR_USE_BOTH
	info.tooltipTitle = MERCHANT_AUTO_REPAIR_USE_BOTH
	info.tooltipText = MERCHANT_AUTO_REPAIR_USE_BOTH_TOOLTIP
	info.tooltipOnButton = true
	info.value = 3
	info.checked = repairSetting == info.value
	info.func = SelectRepairOption
