...
function SetItemButtonCount(button, count)
	...
	if ( count > 1 or (button.isBag and count > 0) ) then
		if button.isBag and ( count > (button.maxDisplayCount or 9999) ) then
			count = "*";
		end
...
function SetItemButtonQuality(button, quality)
	if button.IconBorder then
		...
		if quality then
			if quality >= Enum.ItemQuality.Common and ITEM_QUALITY_COLORS[quality] then
				button.IconBorder:Show();
				button.IconBorder:SetVertexColor(ITEM_QUALITY_COLORS[quality].r, ITEM_QUALITY_COLORS[quality].g, ITEM_QUALITY_COLORS[quality].b);
			else
