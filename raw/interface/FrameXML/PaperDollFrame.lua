...
local CLASS_USE_BLOCK_PER_STRENGTH = {
    ["WARRIOR"] = true,
    ["PALADIN"] = true,
    ["SHAMAN"] = true,
    ["STARCALLER"] = true,
...
RESILIENCE_CRIT_CHANCE_TO_CONSTANT_DAMAGE_REDUCTION_MULTIPLIER = 5.666;

-- update these from this spell effect
--/run GameTooltip:SetOwner(UIParent, "ANCHOR_NONE") GameTooltip:SetPoint("BOTTOMRIGHT", -48, 48) GameTooltip:SetHyperlink("spell:300032")
INT_TO_MELEE_CRIT_PCT = 0.30;
AGI_TO_SPELL_CRIT_PCT = 0.30;

-- this will try to parse the spell to get the percentages
do
	local conversionDesc = GetSpellDescription(300032)
	if conversionDesc and conversionDesc ~= "" then
		local intToMelee, agiToSpell = conversionDesc:match("Converts (%d*%.?%d+)%% Int to melee crit and (%d*%.?%d+)%% Agi to Spell Crit")
		intToMelee = intToMelee and tonumber(intToMelee)
		agiToSpell = agiToSpell and tonumber(agiToSpell)
		if intToMelee and agiToSpell then
			INT_TO_MELEE_CRIT_PCT = intToMelee/100
			AGI_TO_SPELL_CRIT_PCT = agiToSpell/100
		end
	end
end
--Pet scaling:
...
PDFITEMFLYOUT_PLACEINBANK_LOCATION =  0xFFFFFFFC;
PDFITEMFLYOUT_FIRST_SPECIAL_LOCATION = PDFITEMFLYOUT_PLACEINBANK_LOCATION
...
local DRUID_FORM_SPELL_IDS = {
    32356, -- cat form
    32357, -- bear form
    9634, -- dire bear form
    1132356, -- reborn cat form
    1132357, -- reborn bear form
    1109634, -- reborn dire bear form
}

local function IsInFeralForm(unit)
    for _, spellId in pairs(DRUID_FORM_SPELL_IDS) do
        local spellName = GetSpellInfo(spellId)
        if spellName and UnitBuff(unit, spellName) then
            return true
        end
    end
    return false
...
function GetDodgeFromAgility(unit)
    local agility = UnitStat(unit, 2)
    local _, class = UnitClass(unit)
    local perAgi = AGI_PER_DODGE[class] or AGI_PER_DODGE.HERO

    if class == "DRUID" and IsInFeralForm(unit) then
        perAgi = 14.70588
    end

    return agility / perAgi
end
	...
	if ( button == "LeftButton" ) then
		if C_GameMode:IsGameModeActive(Enum.GameMode.BuildDraft) then
			...
			if cursorType == "item" then
				StaticPopup_Show("BUILD_DRAFT_AUTOEQUIP_NOTIFY", link)
				return
			end
		end
		AutoEquipCursorItem();
...
function PaperDollFrame_OnEvent (self, event, ...)
	...
	if ( unit == "player" ) then
		if ( event == "UNIT_LEVEL" ) then
			...
			PaperDollFrame_UpdateHybridConversions()
			return
		end
	end
...
function PaperDollFrame_UpdateHybridConversions()
	local conversionDesc = GetSpellDescription(300032)
	if conversionDesc and conversionDesc ~= "" then
		local intToMelee, agiToSpell = conversionDesc:match("Converts (%d*%.?%d+)%% Int to melee crit and (%d*%.?%d+)%% Agi to Spell Crit")
		intToMelee = intToMelee and tonumber(intToMelee)
...
function PaperDollFrame_SetStat(statFrame, statIndex, unit)
	...
	if unit == "pet" then
		...
		elseif ( statIndex == 2 ) then
			local newLineIndex = strfind(statFrame.tooltip2, "|n")+1;
			statFrame.tooltip2 = strsub(statFrame.tooltip2, 1, newLineIndex);
			statFrame.tooltip2 = format(statFrame.tooltip2, GetCritChanceFromAgility(unit));
		elseif ( statIndex == 3 ) then
			local expectedHealthGain = (((stat - posBuff - negBuff)-20)*10+20)*GetUnitHealthModifier(unit);
			local realHealthGain = ((effectiveStat-20)*10+20)*GetUnitHealthModifier(unit);
			local healthGain = (realHealthGain - expectedHealthGain)*GetUnitMaxHealthModifier(unit);
			statFrame.tooltip2 = format(statFrame.tooltip2, healthGain);
		elseif ( statIndex == 4 ) then
			if ( UnitHasMana(unit) ) then
				local manaGain = ((effectiveStat-20)*15+20)*GetUnitPowerModifier(unit);
				statFrame.tooltip2 = format(statFrame.tooltip2, manaGain, GetSpellCritChanceFromIntellect(unit));
			else
				local newLineIndex = strfind(statFrame.tooltip2, "|n")+2;
				statFrame.tooltip2 = strsub(statFrame.tooltip2, newLineIndex);
				statFrame.tooltip2 = format(statFrame.tooltip2, GetSpellCritChanceFromIntellect(unit));
			end
		elseif ( statIndex == 5 ) then
			statFrame.tooltip2 = format(statFrame.tooltip2, GetUnitHealthRegenRateFromSpirit(unit));
			if ( UnitHasMana(unit) ) then
				statFrame.tooltip2 = statFrame.tooltip2.."\n"..format(MANA_REGEN_FROM_SPIRIT, GetUnitManaRegenRateFromSpirit(unit));
			end
		end
		return
	end
	...
	if ( statIndex == 1 ) then
		...
		if UnitPrimaryStat(unit) == Enum.PrimaryStat.Agility then -- agility exception
			attackPower = math.floor(attackPower/2)
		end

		statFrame.tooltip2 = format(statFrame.tooltip2, attackPower);
		if ( CLASS_USE_BLOCK_PER_STRENGTH[unitClass] ) then
			statFrame.tooltip2 = statFrame.tooltip2 .. "\n" .. format( STAT_BLOCK_TOOLTIP, max(0, effectiveStat*BLOCK_PER_STRENGTH-10) );
		end
	elseif ( statIndex == 3 ) then
		local baseStam = min(20, effectiveStat);
		local moreStam = effectiveStat - baseStam;
		statFrame.tooltip2 = format(statFrame.tooltip2, (baseStam + (moreStam*HEALTH_PER_STAMINA))*GetUnitMaxHealthModifier(unit));
    ...
    elseif ( statIndex == 2 ) then
        local attackPower = GetAttackPowerForStat(statIndex, effectiveStat)
        local meleeCrit   = GetCritChanceFromAgility(unit)
        local dodgeFromAgi = GetDodgeFromAgility(unit)
        ...
        if dodgeFromAgi > 0 then
            statFrame.tooltip2 = statFrame.tooltip2 .. "\n" .. format(DEFAULT_STAT2_DODGE, dodgeFromAgi)
        end
	...
	elseif ( statIndex == 4 ) then
		...
		if ( UnitHasMana(unit) ) then
			local spellCrit = GetSpellCritChanceFromIntellect(unit)
			statFrame.tooltip2 = format(statFrame.tooltip2, baseInt + moreInt*MANA_PER_INTELLECT, spellCrit);
			if C_Player:IsHero() then
...
function PaperDollFrame_SetRating(statFrame, ratingIndex)
	...
	if ( ratingIndex == CR_HIT_MELEE ) then
		local bonus = CR_DYNAMIC_STATS[Enum.StatQueryGroup.MeleeHit]
		local hitCap = HIT_CAP_MELEE
		local playerHitCap = HIT_CAP_MELEE_PVP
		local _, coloredRating = PaperDollFormatCappedRatingStat(rating, ratingBonus,  hitCap - bonus)
		text:SetText(coloredRating);
		local percentText = format("%.2f%% |cff22FF22(+%.2f%%)|r", ratingBonus + bonus, bonus)
		statFrame.tooltip2 = format(CR_HIT_MELEE_TOOLTIP, UnitLevel("player"), percentText, hitCap, playerHitCap);
	elseif ( ratingIndex == CR_HIT_RANGED ) then
		local bonus = CR_DYNAMIC_STATS[Enum.StatQueryGroup.RangedHit]
		local hitCap = HIT_CAP_RANGED
		local playerHitCap = HIT_CAP_RANGED_PVP
		local _, coloredRating = PaperDollFormatCappedRatingStat(rating, ratingBonus, hitCap - bonus)
		text:SetText(coloredRating);
		local percentText = format("%.2f%% |cff22FF22(+%.2f%%)|r", ratingBonus + bonus, bonus)
		statFrame.tooltip2 = format(CR_HIT_RANGED_TOOLTIP, UnitLevel("player"), percentText, hitCap, playerHitCap);
	...
	elseif ( ratingIndex == CR_HIT_SPELL ) then
		local bonus = CR_DYNAMIC_STATS[Enum.StatQueryGroup.SpellHit]
		local hitCap = HIT_CAP_SPELLS
		local playerHitCap = HIT_CAP_SPELLS_PVP
		local _, coloredRating = PaperDollFormatCappedRatingStat(rating, ratingBonus, hitCap - bonus)
		text:SetText(coloredRating);
		local percentText = format("%.2f%% |cff22FF22(+%.2f%%)|r", ratingBonus + bonus, bonus)
		statFrame.tooltip2 = format(CR_HIT_SPELL_TOOLTIP, UnitLevel("player"), percentText, hitCap, playerHitCap);
	...
	elseif ( ratingIndex == CR_EXPERTISE ) then
		text:SetText(GetHitString(rating, ratingBonus, 6.5));
...
function PaperDollFrame_SetResistance(statFrame, statIndex, unit)
	local label = _G[statFrame:GetName().."Label"];
	local text = _G[statFrame:GetName().."StatText"];
	local base, resistance, positive, negative = UnitResistance(unit or "player", statIndex)
	local petBonus = ComputePetBonus("PET_BONUS_RES", resistance)

	local resistanceName = _G["RESISTANCE"..statIndex.."_NAME"];
	label:SetText(resistanceName)
	statFrame.tooltip = format(PAPERDOLLFRAME_TOOLTIP_FORMAT, resistanceName).." "..resistance
	...
	statFrame.tooltip2 = format(RESISTANCE_TOOLTIP_SUBTEXT, _G["RESISTANCE_TYPE"..statIndex], unitLevel, resistanceLevel)
...
function PaperDollFrame_SetItemLevel(statFrame, unit)
	local itemLevel = UnitAverageItemLevel(unit)
	_G[statFrame:GetName().."Label"]:SetText(STAT_ITEM_LEVEL);
	local text = _G[statFrame:GetName().."StatText"];
	...
	text:SetText(color:WrapText(itemLevelText))
	
	statFrame.tooltip = format(AVERAGE_ITEM_LEVEL_S, itemLevelText)
	statFrame.tooltip2 = AVERAGE_ITEM_LEVEL_TOOLTIP
	statFrame:Show();
end

function PaperDollFrame_SetPrimaryStat(statFrame, unit)
	local primaryStat = UnitPrimaryStat(unit)
	statFrame.Label:SetText(PRIMARY_STAT)
	if primaryStat then
		local _, _, _, statName = C_PrimaryStat:GetPrimaryStatInfo(primaryStat)
		statFrame.StatText:SetText(STAT_COLORS[primaryStat]:WrapText(statName))
	else
		statFrame.StatText:SetText(GRAY_FONT_COLOR:WrapText(NO_PRIMARY_STAT))
	end
end

function PaperDollFrame_SetPvEPower(statFrame, unit)
	local pvePower = UnitPvEPower(unit)
	local label = format(STAT_FORMAT, PVE_POWER_LABEL)
	_G[statFrame:GetName().."Label"]:SetText(label);
	local text = _G[statFrame:GetName().."StatText"];
	local color = WHITE_FONT_COLOR
	color = ColorUtil:Lerp(color, ITEM_QUALITY_COLORS[Enum.ItemQuality.Epic], pvePower/PVE_POWER_CAP)
	text:SetText(color:WrapText(pvePower))

	statFrame.tooltip = label .. " " .. pvePower .. "/" .. PVE_POWER_CAP
	statFrame.tooltip2 = {
		PVE_POWER_TOOLTIP1:format(pvePower * PVE_POWER_DAMAGE_MULTIPLIER),
		PVE_POWER_TOOLTIP2:format(pvePower * PVE_POWER_DAMAGE_TAKEN_MULTIPLIER),
		PVE_POWER_TOOLTIP3:format(pvePower * PVE_POWER_HEALING_MULTIPLIER),
	}
	
	local maxLevel = GetMaxLevel()
	if UnitLevel(unit) < maxLevel then
		tinsert(statFrame.tooltip2, 1, POWER_DOES_NOT_APPLY_BELOW_S:format(maxLevel))
	end

	statFrame.minTooltipWidth = 320
	statFrame:Show();
end

function PaperDollFrame_SetPvPPower(statFrame, unit)
	local pvpPower = UnitPvPPower(unit)
	local label = format(STAT_FORMAT, PVP_POWER_LABEL)
	_G[statFrame:GetName().."Label"]:SetText(label);
	local text = _G[statFrame:GetName().."StatText"];
	
	local color = WHITE_FONT_COLOR
	color = ColorUtil:Lerp(color, ITEM_QUALITY_COLORS[Enum.ItemQuality.Epic], pvpPower/PVP_POWER_CAP)
	text:SetText(color:WrapText(pvpPower))

	statFrame.tooltip = label .. " " .. pvpPower .. "/" .. PVP_POWER_CAP
	
	local Line2 = C_Player:IsHero() and PVP_POWER_TOOLTIP2 or PVP_POWER_TOOLTIP2_ALT
	statFrame.tooltip2 = {
		PVP_POWER_TOOLTIP1:format(pvpPower * PVP_POWER_DAMAGE_MULTIPLIER),
		Line2:format(pvpPower * PVP_POWER_HEALING_MULTIPLIER),
		PVP_POWER_TOOLTIP3,
		PVP_POWER_TOOLTIP4:format(pvpPower * PVE_POWER_DAMAGE_MULTIPLIER),
		PVP_POWER_TOOLTIP5:format(pvpPower * PVE_POWER_DAMAGE_TAKEN_MULTIPLIER),
	}
...
function PaperDollFrame_SetPrestigeLevel(statFrame)
	local prestigeLevel, accountPrestigeLevel = GetPrestigeLevel()
	local label = format(STAT_FORMAT, PRESTIGE_LEVEL_LABEL)
	_G[statFrame:GetName().."Label"]:SetText(label);
	local text = _G[statFrame:GetName().."StatText"];
	text:SetText(prestigeLevel)

	statFrame.tooltip = PRESTIGE_LEVEL_LABEL
	statFrame.tooltip2 = {
		PRESTIGE_LEVEL_TOOLTIP:format(accountPrestigeLevel),
		PRESTIGE_LEVEL_TOOLTIP2:format(prestigeLevel),
	}
	statFrame:Show();
end
...
function PaperDollFrame_SetDefense(statFrame, unit)
	...
	local rating, ratingBonus = GetCombatRating(CR_DEFENSE_SKILL), GetCombatRatingBonus(CR_DEFENSE_SKILL)
	local reducedMelee = select(2, GetReducedChanceToBeCrit())
	local _, coloredRating, bonusText = PaperDollFormatCappedRatingStat(rating, defensePercent,  cap + reducedMelee, negBuff, posBuff)
	text:SetText(coloredRating);
	statFrame.tooltip = HIGHLIGHT_FONT_COLOR_CODE..format(PAPERDOLLFRAME_TOOLTIP_FORMAT, DEFENSE).." "..coloredRating..(bonusText or "")..FONT_COLOR_CODE_CLOSE;
	statFrame.tooltip2 = format(DEFAULT_STATDEFENSE_TOOLTIP, rating, ratingBonus, defensePercent, defensePercent);
	statFrame:Show();
end
...
function PaperDollFrame_SetResilience(statFrame)
	...
	statFrame.tooltip2 = format(RESILIENCE_TOOLTIP, lowestRatingBonus * RESILIENCE_CRIT_CHANCE_TO_CONSTANT_DAMAGE_REDUCTION_MULTIPLIER);
	statFrame:Show();
end
...
function PaperDollFrame_GetBonusSpellDamage(unit)
	if unit == "pet" then
		return GetPetSpellBonusDamage()
	end
	local holySchool = 2;
...
function PaperDollFrame_SetSpellBonusDamage(statFrame, unit)
	...
	-- hacky pet handling for stat panel
	if unit == "pet" then
		local damageBonus = GetPetSpellBonusDamage()
		local damageBonusText = format("%d", damageBonus)
		if damageBonus > 0 then
			damageBonusText = GREEN_FONT_COLOR:WrapText("+"..damageBonusText)
		elseif damageBonus < 0 then
			damageBonusText = RED_FONT_COLOR:WrapText("-"..damageBonusText)
		end

		text:SetText(damageBonusText)
		statFrame.tooltip = HIGHLIGHT_FONT_COLOR:WrapText(format(PAPERDOLLFRAME_TOOLTIP_FORMAT, SPELL_BONUS)).." "..damageBonusText
		statFrame.tooltip2 = DEFAULT_STATSPELLBONUS_TOOLTIP
		return
	end
...
function PaperDollFrame_SetSpellCritChance(statFrame)
	...
	statFrame.minCrit = minCrit;
	statFrame.onEnterFunc = CharacterSpellCritChance_OnEnter
	statFrame:Show();
end
...
function PaperDollFrame_SetMeleeCritChance(statFrame)
	...
	local tooltip = CR_CRIT_MELEE_TOOLTIP;
	if select(2, UnitClass(statFrame.unit or "player")) ~= "HERO" then
		tooltip = CR_CRIT_MELEE_TOOLTIP_ALT;
	end
	statFrame.tooltip2 = format(tooltip, GetCombatRating(CR_CRIT_MELEE), GetCombatRatingBonus(CR_CRIT_MELEE));
end

function PaperDollFrame_SetMeleeOffHandHit(statFrame)
	local statName = MELEE_OFF_HAND_HIT
	_G[statFrame:GetName().."Label"]:SetText(format(STAT_FORMAT, statName));
	local text = _G[statFrame:GetName().."StatText"];
	...
	statFrame.tooltip = HIGHLIGHT_FONT_COLOR_CODE..format(PAPERDOLLFRAME_TOOLTIP_FORMAT, statName).." "..rating..FONT_COLOR_CODE_CLOSE;
	local bonus = CR_DYNAMIC_STATS[Enum.StatQueryGroup.MeleeHit]
	bonus = bonus + CR_DYNAMIC_STATS[Enum.StatQueryGroup.OffHandHit]
	local hitCap = HIT_CAP_AUTO_ATTACK
	local playerHitCap = HIT_CAP_AUTO_ATTACK_PVP
	local _, coloredRating = PaperDollFormatCappedRatingStat(rating, ratingBonus,  hitCap - bonus)
	text:SetText(coloredRating);
	local percentText = format("%.2f%% |cff22FF22(+%.2f%%)|r", ratingBonus + bonus, bonus)
	statFrame.tooltip2 = format(CR_HIT_MELEE_OFF_HAND_TOOLTIP, UnitLevel("player"), percentText, hitCap, playerHitCap);
end
...
function PaperDollFrame_SetRangedCritChance(statFrame)
	...
	local tooltip = CR_CRIT_RANGED_TOOLTIP;
	if select(2, UnitClass(statFrame.unit or "player")) ~= "HERO" then
		tooltip = CR_CRIT_RANGED_TOOLTIP_ALT;
	end
	statFrame.tooltip2 = format(tooltip, GetCombatRating(CR_CRIT_RANGED), GetCombatRatingBonus(CR_CRIT_RANGED));
end
...
function PaperDollFrame_SetExpertise(statFrame, unit)
	...
	local speed, offhandSpeed = UnitAttackSpeed(unit);
	local tooltipText;
	local _, text = PaperDollFormatCappedRatingStat(expertise, expertisePercent, 6.5)
	tooltipText = text
	if offhandSpeed then
		local _, ohText = PaperDollFormatCappedRatingStat(offhandExpertise, offhandExpertisePercent, 6.5)
		text = text .. " / " .. ohText
		tooltipText = text .. "  (" .. ohText .. ")"
	end
	PaperDollFrame_SetLabelAndText(statFrame, STAT_EXPERTISE, text);

	statFrame.tooltip = HIGHLIGHT_FONT_COLOR_CODE..format(PAPERDOLLFRAME_TOOLTIP_FORMAT, _G["COMBAT_RATING_NAME"..CR_EXPERTISE])..FONT_COLOR_CODE_CLOSE.." "..tooltipText;
...
function CharacterSpellBonusDamage_OnEnter (self)
	...
	if self.unit and select(2, UnitClass(self.unit)) == "HERO" then
		GameTooltip:AddLine("\n" .. format(MELEE_ATTACK_POWER_TOOLTIP, self.minModifier * BONUS_WEAPON_DAMAGE_PER_SPELLPOWER), NORMAL_FONT_COLOR.r, NORMAL_FONT_COLOR.g, NORMAL_FONT_COLOR.b, true)
		GameTooltip:AddLine(format(RANGED_ATTACK_POWER_TOOLTIP, self.minModifier * BONUS_WEAPON_DAMAGE_PER_SPELLPOWER), NORMAL_FONT_COLOR.r, NORMAL_FONT_COLOR.g, NORMAL_FONT_COLOR.b, true)
	end
	GameTooltip:Show();
end
...
function CharacterSpellCritChance_OnEnter (self)
	...
	local tooltip = CR_CRIT_SPELL_TOOLTIP;
	if self.unit and select(2, UnitClass(self.unit)) ~= "HERO" then
		tooltip = CR_CRIT_SPELL_TOOLTIP_ALT;
	end
...
function PaperDollFrame_OnShow (self)
	...
	C_Hook:RegisterBucket(self, "UNIT_AURA", 0.1, function()
		PaperDollFrame_OnEvent(self, "UNIT_AURA")
	end)

	C_Hook:RegisterBucket(self, "SPELLS_CHANGED", 0.1, function()
		PaperDollFrame_OnEvent(self, "SPELLS_CHANGED")
	end)
...
function PaperDollFrame_OnHide (self)
	...
	C_Hook:Unregister(self, "SPELLS_CHANGED")
	C_Hook:Unregister(self, "ASCENSION_HIDDEN_SPELL_LEARNED")
	C_Hook:Unregister(self, "ASCENSION_HIDDEN_SPELL_UNLEARNED")
end
...
function PaperDollItemSlotButton_OnClick (self, button)
	...
	if ( button == "LeftButton" ) then
		...
		if ( type == "merchant" and MerchantFrame.extendedCost ) then
			...
			if C_GameMode:IsGameModeActive(Enum.GameMode.BuildDraft) then
				if type == "item" then
					StaticPopup_Show("BUILD_DRAFT_SWAPEQUIP_NOTIFY", link, nil, self)
					return
				end
			end
...
function PaperDollFormatCappedRatingStat(value, percentage, requiredPercent, negBuff, posBuff)
	local text
	local longText

	local bonusText
	if negBuff and negBuff < 0 then
		bonusText = string.format("|cffffffff(-%s|cffffffff)|r", RED_FONT_COLOR:WrapText(posBuff))
	elseif posBuff and posBuff > 0 then
		bonusText = string.format("|cffffffff(+%s|cffffffff)|r", GREEN_FONT_COLOR:WrapText(posBuff))
	end
	
	if requiredPercent <= 0 then
		longText = "|cff85FF85" .. value .. "|r"
		text = "|cff85FF85" .. value .. "|r"
		return text, longText, bonusText
	end
	
	if value > 0 and percentage > 0 then
		local percentPerPoint = percentage / value
		local requiredAmount = ceil(requiredPercent / percentPerPoint)
	
		if value < requiredAmount then
			text = "|cffFF6B6B"
		else
			text = "|cff85FF85"
		end
	
		longText = text .. value .. "/" .. requiredAmount .. "|r"
		text = text .. value .. "|r"
	else
		longText = "|cffFF6B6B" .. value .. "|r"
		text = "|cffFF6B6B" .. value .. "|r"
	end
...
function CharacterDamageFrame_OnEnter (self)
	-- Main hand weapon
	GameTooltip:SetOwner(self, "ANCHOR_RIGHT");
	if ( self == PetDamageFrame or self.unit == "pet" ) then
	...
	GameTooltip_AddSpacer(GameTooltip)
	local tooltip = CR_MELEE_DAMAGE_TOOLTIP;
	if self.unit and select(2, UnitClass(self.unit)) ~= "HERO" then
		tooltip = CR_MELEE_DAMAGE_TOOLTIP_ALT;
	end
	GameTooltip:AddLine(tooltip, NORMAL_FONT_COLOR.r, NORMAL_FONT_COLOR.g, NORMAL_FONT_COLOR.b, true)
	GameTooltip:Show();
end
...
function CharacterRangedDamageFrame_OnEnter (self)
	...
	GameTooltip:AddDoubleLine(format(STAT_FORMAT, DAMAGE_PER_SECOND), format("%.1f", self.dps), NORMAL_FONT_COLOR.r, NORMAL_FONT_COLOR.g, NORMAL_FONT_COLOR.b, NORMAL_FONT_COLOR.r, NORMAL_FONT_COLOR.g, NORMAL_FONT_COLOR.b);
	GameTooltip_AddSpacer(GameTooltip)
	local tooltip = CR_RANGED_DAMAGE_TOOLTIP;
	if self.unit and select(2, UnitClass(self.unit)) ~= "HERO" then
		tooltip = CR_RANGED_DAMAGE_TOOLTIP_ALT;
	end
	GameTooltip:AddLine(tooltip, NORMAL_FONT_COLOR.r, NORMAL_FONT_COLOR.g, NORMAL_FONT_COLOR.b, true)
	GameTooltip:Show();
end
...
function UpdatePaperdollStats(prefix, index)
	...
	elseif ( index == "PLAYERSTAT_MELEE_COMBAT" ) then
		...
		if select(2, UnitAttackSpeed("player")) then
			PaperDollFrame_SetMeleeOffHandHit(stat5);
...
function PaperDollFrameItemFlyout_Show (paperDollItemSlot)
	...
	if ( paperDollItemSlot.hasItem ) then
		...
		tinsert(itemDisplayTable, 2, PDFITEMFLYOUT_PLACEINBANK_LOCATION);
		numItems = numItems + 1;
	end
...
function PaperDollFrameItemFlyout_DisplaySpecialButton (button, paperDollItemSlot)
	...
	elseif ( location == PDFITEMFLYOUT_PLACEINBAGS_LOCATION ) then
		...
		SetItemButtonTextureVertexColor(button, 1.0, 1.0, 1.0);
		SetItemButtonNormalTextureVertexColor(button, 1.0, 1.0, 1.0);
	elseif ( location == PDFITEMFLYOUT_PLACEINBANK_LOCATION ) then
		SetItemButtonTexture(button, "Interface\\PaperDollInfoFrame\\UI-GearManager-ItemIntoBank");
		SetItemButtonCount(button, nil);
		button.UpdateTooltip =
		function ()
			GameTooltip:SetOwner(PaperDollFrameItemFlyoutButtons, "ANCHOR_RIGHT", 6, -PaperDollFrameItemFlyoutButtons:GetHeight() - 6);
			GameTooltip:SetText(EQUIPMENT_MANAGER_PLACE_IN_BANK, 1.0, 1.0, 1.0);
			if ( SHOW_NEWBIE_TIPS == "1" ) then
...
function PaperDollFrameItemFlyoutButton_OnClick (self)
	...
	elseif ( self.location == PDFITEMFLYOUT_PLACEINBANK_LOCATION ) then
		if ( UnitAffectingCombat("player") and not INVSLOTS_EQUIPABLE_IN_COMBAT[PaperDollFrameItemFlyout.button:GetID()] ) then
			UIErrorsFrame:AddMessage(ERR_CLIENT_LOCKED_OUT, 1.0, 0.1, 0.1, 1.0);
			return;
		end
		local action = EquipmentManager_UnequipItemInSlot(PaperDollFrameItemFlyout.button:GetID(), true);
		EquipmentManager_RunAction(action);
...
function GearManagerToggleButton_OnLoad (self)
	self.icon = _G[self:GetName() .. "Icon"];
end

-- Disable Gear Manager while in queue for BGs or in BGs
function GearManagerToggleButton_CheckCanUse (self)
	for i = 1, MAX_BATTLEFIELD_QUEUES do
		local status, _, _, _, _, teamSize = GetBattlefieldStatus(i)
		if status ~= "none" and teamSize == 0 then
			self:Disable()
	...
	if C_Instance.IsInBattleground() then
		self:Disable()
		self:GetNormalTexture():SetDesaturated(true)
		return
	end
...
function GearManagerToggleButton_OnShow (self)
	self:RegisterEvent("UPDATE_BATTLEFIELD_STATUS")
	GearManagerToggleButton_CheckCanUse(self)
end

function GearManagerToggleButton_OnHide (self)
	self:UnregisterEvent("UPDATE_BATTLEFIELD_STATUS")
end

function GearManagerToggleButton_OnEvent(self, event, ...)
	GearManagerToggleButton_CheckCanUse(self)
end
...
function GearManagerDialogSendItemsToBank_OnClick (self)
	PlaySound(SOUNDKIT.CHAT_SCROLL_BUTTON)
	local checked = toboolean(self:GetChecked())
	C_CVar.Set("bankEquipmentManager", checked and "1" or "0")
end
...
function GetEquipmentSetIconCount()
	return _numItems + GetNumMacroIcons();
end
...
function PlayerTitleFrame_UpdateTitles()
	...
	playerTitles[1].name = "       ";
	playerTitles[1].id = -1;		
	for i = 1, GetNumTitles() do -- GetNumTitles returns + 1 higher than it should!
...
function PaperDollFrame_SetSoloManastorm(statFrame, unit)
	local _, solo = C_Manastorm.GetMaxCompletedLevels(unit)
	_G[statFrame:GetName().."Label"]:SetText(HIGHEST_SOLO_MANASTORM_LEVEL);
	local text = _G[statFrame:GetName().."StatText"];
	text:SetText(solo or 0)
...
function PaperDollFrame_SetDuoManastorm(statFrame, unit)
	local _, _, duo = C_Manastorm.GetMaxCompletedLevels(unit)
	_G[statFrame:GetName().."Label"]:SetText(HIGHEST_DUO_MANASTORM_LEVEL);
	local text = _G[statFrame:GetName().."StatText"];
	text:SetText(duo or 0)
...
function PaperDollFrame_SetTrioManastorm(statFrame, unit)
	local _, _, _, trio = C_Manastorm.GetMaxCompletedLevels(unit)
	_G[statFrame:GetName().."Label"]:SetText(HIGHEST_TRIO_MANASTORM_LEVEL);
	local text = _G[statFrame:GetName().."StatText"];
	text:SetText(trio or 0)
...
function PaperDollFrame_SetGroupManastorm(statFrame, unit)
	local _, _, _, _, group = C_Manastorm.GetMaxCompletedLevels(unit)
	_G[statFrame:GetName().."Label"]:SetText(HIGHEST_GROUP_MANASTORM_LEVEL);
	local text = _G[statFrame:GetName().."StatText"];
	text:SetText(group or 0)
