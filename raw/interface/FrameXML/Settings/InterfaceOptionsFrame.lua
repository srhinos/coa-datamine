...
uvarInfo = {
	...
	["COMBAT_TEXT_SHOW_INCOMING_DAMAGE"] = { default = "0", cvar = "fctIncomingDamage", event = "COMBAT_TEXT_SHOW_INCOMING_DAMAGE_TEXT" },
	["COMBAT_TEXT_SHOW_OUTGOING_DAMAGE"] = { default = "0", cvar = "fctOutgoingDamage", event = "COMBAT_TEXT_SHOW_OUTGOING_DAMAGE_TEXT" },
	...
	["SPELL_ACTIVATION_OVERLAY_ENABLED"] = { default = "1", cvar = "SpellActivationOverlayEnabled", event = "ENABLE_SPELL_ACTIVATION_OVERLAY" },
}
...
function InterfaceOptionsFrame_OnShow (self)
	if C_GameMode:IsGameModeActive(Enum.GameMode.Draft, Enum.GameMode.BuildDraft) then
		InterfaceOptions_AddCategory(InterfaceOptionsDraftPanel)
	else
		InterfaceOptions_RemoveCategory(InterfaceOptionsDraftPanel)
	end
...
function InterfaceOptions_AddCategory (frame, addOn, position)
	-- Check if the frame is already in the blizzardCategories
	for _, category in ipairs(blizzardCategories) do
		if category == frame then
			return; -- If the frame is already in the list, return immediately
		end
	end
...
function InterfaceOptions_RemoveCategory(frame)
	-- Check if the frame is a Blizzard panel
	for i, category in ipairs(blizzardCategories) do
		if category == frame then
			table.remove(blizzardCategories, i)
			InterfaceCategoryList_Update()
			return
		end
	end

	-- If not a Blizzard panel, check if it's an AddOn panel
