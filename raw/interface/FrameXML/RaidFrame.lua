...
function RaidFrame_OnEvent(self, event, ...)
	...
	elseif ( event == "UPDATE_INSTANCE_INFO" ) then
		...
		if ( GetNumSavedInstances() > 0 or next(GetLootLockouts("player")) ) then
...
function RaidFrame_Update()
	...
	if ( GetNumRaidMembers() == 0 ) then
		...
		if ( GetPartyMember(1) and IsPartyLeader() and not HasLFGRestrictions() ) then
...
function RaidInfoFrame_Update(scrollToSelected)
	...
	local instanceLockouts, numInstanceLockouts = C_Instance:GetInstanceLocks()
	...
	for i=1, numButtons do
		...
		if ( index <=  numInstanceLockouts) then
			local instance = instanceLockouts[index]

			frame.mapID = instance.mapID

			frame.lockout = instance.lockout
			frame.instanceID = instance.instanceID;
			frame.longInstanceID = string.format("%x%x", instance.instanceIDMostSig, instance.instanceID);
			frame:SetID(index);
			...
			frame.difficulty:SetText(instance.difficultyName);

			if ( instance.extended or instance.locked ) then
				frame.reset:SetText(SecondsToTime(instance.instanceReset, true, nil, 3));
				frame.name:SetText(instance.instanceName);
			else
				frame.reset:SetFormattedText("|cff808080%s|r", RAID_INSTANCE_EXPIRES_EXPIRED);
				frame.name:SetFormattedText("|cff808080%s|r", instance.instanceName);
			end

			if ( instance.extended ) then
				frame.extended:Show();
			else
				frame.extended:Hide();
			end
	...
	HybridScrollFrame_Update(scrollFrame, numInstanceLockouts * buttonHeight, scrollFrame:GetHeight());
end
...
function RaidInfoInstance_OnEnter(self)
	...
	GameTooltip:AddLine(self.difficulty:GetText(), HIGHLIGHT_FONT_COLOR:GetRGB());
	if self.instanceID and self.instanceID >= 0 then
		GameTooltip:AddLine(format(INSTANCE_ID, self.instanceID), HIGHLIGHT_FONT_COLOR:GetRGB());
	else
		local r, g, b = ITEM_QUALITY_COLORS[9]:GetRGB()
		GameTooltip:AddLine(RAID_RESET_LOCATION_HINT, r, g, b, true)
	end

	if self.lockout then
		GameTooltip_AddSpacer(GameTooltip)
		for encounterID, info in pairs(self.lockout) do
			local color = info.TimeRemaining > 0 and RED_FONT_COLOR or GREEN_FONT_COLOR
			local defeated = info.TimeRemaining > 0 and format("Locked (%s)", SecondsToTime(info.TimeRemaining, true, false, 2)) or "Undefeated"
			GameTooltip:AddDoubleLine(info.EncounterName, defeated, 1, 1, 1, color:GetRGB())
		end
	end
