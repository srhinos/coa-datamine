...
UnitPopupButtons["PET_TALENTS"] = {text = TALENT_SPEC_PET_PRIMARY, dist = 0};
...
UnitPopupButtons["RESET_INSTANCES"] = { text = RESET_INSTANCES, dist = 0, nested = 1 };
UnitPopupButtons["RESET_ALL_INSTANCES"] = { text = RESET_ALL_INSTANCES, dist = 0 };
UnitPopupButtons["RESET_ALL_RAIDS"] = { text = RESET_ALL_RAIDS, dist = 0 };
UnitPopupButtons["RESET_ALL_DUNGEONS"] = { text = RESET_ALL_DUNGEONS, dist = 0 };
UnitPopupButtons["RESET_INSTANCE_LIST"] = { dist = 0 };
...
UnitPopupButtons["PVP_PVE_RISK"] = { text = NO_RISK_PVE, dist = 0, checkable = 1 };
UnitPopupButtons["PVP_PVP_RISK"] = { text = NO_RISK_PVP, dist = 0, checkable = 1 };
UnitPopupButtons["PVP_HIGH_RISK"] = { text = HIGH_RISK_PVP, dist = 0, checkable = 1 };

UnitPopupButtons["TOGGLE_DRAGON"] = { text = "Artwork", dist = 0, nested = 1 }
...
-- Second level menus
UnitPopupMenus["PVP_FLAG"] = { "PVP_PVE_RISK", "PVP_PVP_RISK", "PVP_HIGH_RISK"};
UnitPopupMenus["TOGGLE_DRAGON"] = { "SHOW_DRAGON", "HIDE_DRAGON"};
UnitPopupMenus["RESET_INSTANCES"] = { "RESET_ALL_DUNGEONS", "RESET_INSTANCE_LIST" }; -- "RESET_ALL_RAIDS"
...
function UnitPopup_ShowMenu (dropdownMenu, which, unit, name, userData)
	...
	local specIndex, specName = ClassInfoUtil.GetDefaultLootSpecID()
	if specIndex and specName then
	...
	for index = 1, 4 do
		...
		if ( specPopupButton ) then
			if class == "HERO" then
				local _, _, icon, statName = C_PrimaryStat:GetPrimaryStatInfo(index)
				specPopupButton.specializationID = -index; -- negative primary stat index
				specPopupButton.text = CreateSquareTextureMarkup(icon, 20) ..  statName
			else
				local specInfo = ClassInfoUtil.GetSpecInfoByIndex(class, index)
				if specInfo then
					local id, name = specInfo.ID, specInfo.Name
					if ( id ) then
						specPopupButton.specializationID = id;
						specPopupButton.text = CreateSquareTextureMarkup("Interface\\Icons\\"..specInfo.SpecFilename, 20) .. " " .. name;
					else
						specPopupButton.specializationID = false;
					end
				else
					specPopupButton.specializationID = false;
				end
	...
	if ( UIDROPDOWNMENU_MENU_LEVEL == 2 ) then
		...
		for index, value in ipairs(UnitPopupMenus[UIDROPDOWNMENU_MENU_VALUE]) do
			if( UnitPopupShown[UIDROPDOWNMENU_MENU_LEVEL][index] == 1 ) then
				...
				elseif ( value == "PVP_PVE_RISK" ) then
					local ruleset =  C_Player:GetRuleset()
					if ( ruleset == Enum.Ruleset.NoRiskPvE or ruleset == Enum.Ruleset.None ) then
						info.checked = 1;
					end
				elseif ( value == "PVP_PVP_RISK" ) then
					if ( C_Player:GetRuleset() == Enum.Ruleset.NoRiskPvP ) then
						info.checked = 1;
					end
					info.disabled = C_Player:GetLevel() < 15
				elseif ( value == "PVP_HIGH_RISK" ) then
					if ( C_Player:GetRuleset() == Enum.Ruleset.HighRiskPvP ) then
						info.checked = 1;
					end
					info.disabled = C_Player:GetLevel() < 15
				elseif ( value == "SHOW_DRAGON" ) then
					if ( C_CVar.GetBool("showPortraitDragon") ) then
						info.checked = 1;
					end
				elseif ( value == "HIDE_DRAGON" ) then
					if ( not C_CVar.GetBool("showPortraitDragon") ) then
						info.checked = 1;
					end
				elseif value == "RESET_INSTANCE_LIST" then
					for _, lockout in ipairs(C_Instance:GetSavedMapAndDifficulty()) do
						info.text = format("%s (%s)", GetMapName(lockout.mapID) or lockout.mapID, _G["GENERIC_DIFFICULTY"..lockout.difficultyID] or lockout.difficultyID)
						info.value = format("RESET_INSTANCE_LIST_%d_%d", lockout.mapID, lockout.difficultyID);
				...
				if value ~= "RESET_INSTANCE_LIST" then
					info.value = value;
					info.func = UnitPopup_OnClick;
					-- Setup newbie tooltips
	...
	for index, value in ipairs(UnitPopupMenus[UIDROPDOWNMENU_MENU_VALUE] or UnitPopupMenus[dropdownMenu.which]) do
		...
		elseif ( value == "INVITE" ) then
			if ( dropdownMenu.unit ) then
				...
				if ( UnitIsUnit("player", dropdownMenu.unit) or (server and server ~= "") ) then
					UnitPopupShown[UIDROPDOWNMENU_MENU_LEVEL][index] = 0;
				end
		...
		elseif ( value == "SELECT_LOOT_SPECIALIZATION" ) then
			if ( not ClassInfoUtil.HasLootSpec() ) then
				UnitPopupShown[UIDROPDOWNMENU_MENU_LEVEL][index] = 0;
			end
		elseif ( strsub(value, 1, 20) == "LOOT_SPECIALIZATION_" ) then
			if ( not UnitPopupButtons[value].specializationID ) then
				UnitPopupShown[UIDROPDOWNMENU_MENU_LEVEL][index] = 0;
			end
		...
		elseif ( value == "PET_TALENTS" ) then
			if ( not PetCanBeAbandoned() or GetNumTalents(1, false, true) == 0 ) then
				UnitPopupShown[UIDROPDOWNMENU_MENU_LEVEL][index] = 0;
			end
		elseif ( strsub(value, 1, 12)  == "RAID_TARGET_" ) then
		...
		if ( dropdownFrame ) then
			...
			for index, value in ipairs(UnitPopupMenus[dropdownFrame.which]) do				
				if ( UnitPopupShown[level][index] == 1 ) then
					...
					elseif ( value == "INVITE" ) then
						if ( currentDropDown.server ) then
							enable = 0;
						end
					elseif ( value == "UNINVITE" ) then
						if ( inParty == 0 or (isLeader == 0) or HasLFGRestrictions() ) then
...
function UnitPopup_OnClick (self)
	...
	elseif ( button == "PET_TALENTS" ) then
		C_Quest:SendPathToAscensionEvent("ACTION_OPEN_PET_TALENTS")
		ToggleTalentFrame()
	...
	elseif ( button == "PVP_PVE_RISK" ) then
		C_Player:SetRuleset(Enum.Ruleset.NoRiskPvE)
	elseif ( button == "PVP_PVP_RISK" ) then
		C_Player:SetRuleset(Enum.Ruleset.NoRiskPvP)
	elseif ( button == "PVP_HIGH_RISK" ) then
		C_Player:SetRuleset(Enum.Ruleset.HighRiskPvP)
	elseif ( button == "RESET_ALL_RAIDS" ) then
		StaticPopup_Show("CONFIRM_RESET_RAIDS");
	elseif ( button == "RESET_ALL_DUNGEONS" ) then
		StaticPopup_Show("CONFIRM_RESET_DUNGEONS");
	elseif button:startswith("RESET_INSTANCE_LIST") then
		local mapID, difficultyID = button:match("RESET_INSTANCE_LIST_(%d+)_(%d+)")
		mapID = tonumber(mapID)
		difficultyID = tonumber(difficultyID)
		if not mapID or not difficultyID then
			C_Logger.Error("Could not get mapID or difficultyID from button: " .. (button or "nil"))
			return
		end
	...
	elseif ( button == "HIDE_DRAGON" ) then
		C_CVar.Set("showPortraitDragon", "0")
		if not UnitHasVehicleUI("player") then
			PlayerFrame_ToPlayerArt(PlayerFrame)
		end
