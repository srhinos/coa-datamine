...
CLASS_BUTTONS = {
	...
	["HERO"]		= {0.7421875, 0.98828125, 0, 0.25},
	["BARBARIAN"]	= { 1, 1, 1, 1 },
	["WITCHDOCTOR"] = { 1, 1, 1, 1 },
	["DEMONHUNTER"] = { 1, 1, 1, 1 },
	["WITCHHUNTER"] = { 1, 1, 1, 1 },
	["STORMBRINGER"]= { 1, 1, 1, 1 },
	["FLESHWARDEN"] = { 1, 1, 1, 1 },
	["GUARDIAN"] 	= { 1, 1, 1, 1 },
	["MONK"] 		= { 1, 1, 1, 1 },
	["SONOFARUGAL"] = { 1, 1, 1, 1 },
		...
		if ( (uiType ~= 1) or ((WORLD_PVP_OBJECTIVES_DISPLAY == "1") or (WORLD_PVP_OBJECTIVES_DISPLAY == "2" and IsSubZonePVPPOI()) or (instanceType == "pvp")) ) then
			if ( state > 0 ) then
				...
				if ( extendedUI ~= "" ) then
					...
					if not(uiInfo) then
						uiInfo = ExtendedUI["CAPTUREPOINT"]
					end
