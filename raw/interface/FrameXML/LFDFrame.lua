...
local LFG_DUNGEONS_SCALING = { [1]=true, [4]=true, [6]=true, [8]=true, [10]=true, [12]=true, [16]=true, [14]=true, [18]=true, [165]=true, [20]=true, [163]=true, [22]=true, [164]=true, [272]=true, [24]=true, [26]=true, [273]=true, [28]=true };

function GetSelectedDungeons()
	local dungeons = "";

	local offset = FauxScrollFrame_GetOffset(LFDQueueFrameSpecificListScrollFrame);
	for i = 2, NUM_LFD_CHOICE_BUTTONS do
		local button = _G["LFDQueueFrameSpecificListButton"..i.."EnableButton"];
		local dungeonID = LFDDungeonList[i+offset];
		if ( dungeonID and button:GetChecked() ) then
			if ( dungeons == "" ) then
				dungeons = dungeons..dungeonID;
			else
				dungeons = dungeons.." "..dungeonID;
			end
		end
...
function LFDFrame_OnEvent(self, event, ...)
	...
	elseif ( event == "LFG_PROPOSAL_SHOW" ) then
        ...
        LFGDebug("Proposal Shown: LFDFrame_OnEvent LFG_PROPOSAL_SHOW ");
...
function LFDRoleCheckPopup_Update()
	...
	if not displayName then
		--@andrew ? keeps happening while queuing fake random classic. Unsure of cause but this will fix when it cant resolve a name
		displayName = A_RANDOM_DUNGEON 
	end
...
function LFDQueueFrameSpecificListButton_SetDungeon(button, dungeonID, mode, submode)
	...
	if ( LFGIsIDHeader(dungeonID) ) then
		...
		if ( LFG_DUNGEONS_SCALING[dungeonID] ) then
...
function LFDQueueFrameDungeonListButton_OnEnter(self)
	...
	if ( self.lockedIndicator:IsShown() ) then
		if ( LFGIsIDHeader(dungeonID) ) then
			...
			GameTooltip:SetText(YOU_MAY_NOT_QUEUE_FOR_DUNGEON, 1.0, 1.0, 1.0);
			local events = C_LFG.RequiresGameEvent[dungeonID]
			if events and type("events") ~= "table" then
				events = { events }
			end
			if events and not GameEventUtil.IsAnyEventActive(unpack(events)) then
				GameTooltip:AddLine(INSTANCE_UNAVAILABLE_DIFFICULTY_NOT_RELEASED)
			else
...
function LFDDungeonReadyDialogReward_SetReward(button, dungeonID, rewardIndex)
	if button then -- TODO: Disabled until rewards can be reworked.
		button:Hide()
	end
...
function LFDQueueFrameFindGroupButton_OnEnter()
	local leader, tank, healer, dps = GetLFGRoles()

	if not tank and not healer and not dps then
		HelpTip:Show("LFD_ROLE_SELECT_HELP")
	end
end

function LFDQueueFrameFindGroupButton_OnLeave()
...
function LFDQueueFrame_Update()
	...
	LFDQueueFrameSpecificList_Update();
	local customLFD = _G["AscensionPVEFrameLFDFrame"]
	if customLFD then
		customLFD:UpdateSpecific()
	end
end
...
function LFDList_DefaultFilterFunction(dungeonID)
	...
	local groupID = info[LFG_RETURN_VALUES.groupID]
	...
	return  (hasHeader and sufficientExpansion and sufficientLevel) and
			(level - LFD_MAX_SHOWN_LEVEL_DIFF <= info[LFG_RETURN_VALUES.recLevel]) and
			(not LFGLockList or not LFGLockList[groupID] or LFGLockList[groupID] == 0)
end
