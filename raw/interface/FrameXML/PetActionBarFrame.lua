...
function PetActionBar_UpdatePositionValues()
	...
	elseif ( MultiCastActionBarFrame and HasMultiCastActionBar() ) then
		PETACTIONBAR_XPOS = MULTICASTACTIONBAR_XPOS + (MultiCastActionBarFrame.numActiveSlots * 58) + 20;
	elseif ( ShapeshiftBarFrame and GetNumShapeshiftForms() > 0 ) then
        local xOffset = (ShapeshiftBarFrame:GetWidth() + 8) * GetNumShapeshiftForms()
		PETACTIONBAR_XPOS = xOffset + 20;
	else
		PETACTIONBAR_XPOS = 36;
	end
end
...
function PetActionButton_OnLoad (self)
	self.buttonType = "BONUSACTIONBUTTON"
...
function PetActionButton_OnLeave(self)
...
function PetActionButton_SetHotkeys (self)
	ActionButton_UpdateHotkeys(self, "BONUSACTIONBUTTON")
end
