...
function TotemFrame_OnLoad(self)
	...
	if classMask and bit.contains(bit.bor(Enum.ClassMask.HERO, Enum.ClassMask.DEATHKNIGHT, Enum.ClassMask.COA), classMask) then
		self:SetPoint("TOPLEFT", PlayerFrame, "TOPLEFT", 65, -75);
	end
end

function TotemFrame_Update()
	local classMask = UnitClassMask("player")
	local shouldShow = classMask and bit.contains(bit.bor(Enum.ClassMask.HERO, Enum.ClassMask.DEATHKNIGHT, Enum.ClassMask.COA), classMask)
	if shouldShow then
		if RuneFrame and RuneFrame:IsShown() then
			TotemFrame:SetPoint("TOPLEFT", PlayerFrame, "TOPLEFT", 28, -100);
		else
			TotemFrame:SetPoint("TOPLEFT", PlayerFrame, "TOPLEFT", 65, -75);
		end
	end
...
function TotemFrame_OnShow()
	PetFrame_Update(PetFrame)
end

function TotemFrame_OnHide()
	PetFrame_Update(PetFrame)
end
