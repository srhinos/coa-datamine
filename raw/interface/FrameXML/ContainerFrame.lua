...
function ContainerFrame_OnShow(self)
    ...
    C_Hook:Register(self, "NEW_BAG_ITEM_ADDED", function(bag)
...
function ContainerFrame_Update(frame)
    ...
    local newItemTexture, flash, newItemAnim, flashAnim
	...
	for i=1, frame.size, 1 do
        ...
        local isNewItem = C_CVar.GetBool("HighlightNewItems") and C_InventoryState:IsNewItem(id, itemButton:GetID())

		newItemTexture = _G[name.."Item"..i].NewItemTexture
		flash = _G[name.."Item"..i].Flash
		newItemAnim = newItemTexture.Anim
        flashAnim = flash.Anim

		if ( isNewItem ) then
			if (quality and NEW_ITEM_ATLAS_BY_QUALITY[quality]) then
				newItemTexture:SetAtlas(NEW_ITEM_ATLAS_BY_QUALITY[quality]);
			else
				newItemTexture:SetAtlas("bags-glow-white", Const.TextureKit.IgnoreAtlasSize);
			end
			newItemTexture:Show();
    ...
    if IsControlKeyDown() and IsAltKeyDown() then
        ...
        if itemID then
            ...
            if appearanceID and not C_AppearanceCollection.IsAppearanceCollected(appearanceID) then
                local guid = GetContainerItemGUID(self:GetParent():GetID(), self:GetID())
                StaticPopup_Show("CONFIRM_COLLECT_APPEARANCE", AppearanceUtil.GetAppearanceLink(appearanceID), nil, guid)
                return
            end
        end
    end
...
function ContainerFrameItemButton_OnEnter(self)
	...
	local itemID = self.hasItem and GetContainerItemID(self:GetParent():GetID(), self:GetID())
	local appearanceID = itemID and C_Appearance.GetItemAppearanceID(itemID)

	if self.hasItem and appearanceID and not showSell and IsControlKeyDown() and IsAltKeyDown() and not C_AppearanceCollection.IsAppearanceCollected(appearanceID) then
		SetCursor("Interface\\CURSOR\\Transmogrify")
    ...
    C_InventoryState:RemoveNewItem(self:GetParent():GetID(), self:GetID())

    if self.Flash.Anim:IsPlaying() or self.NewItemTexture.Anim:IsPlaying() then
		self.Flash.Anim:Stop()
		self.NewItemTexture.Anim:Stop()
	end
end
