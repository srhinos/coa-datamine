...
function MultiCastFlyoutFrame_LoadPageSpells(self)
    ...
    for i, spellId in next, TOTEM_MULTI_CAST_SUMMON_SPELLS_ALT do
        if ( knownMultiCastSummonSpells[i] and not knownIndexes[i] ) then
            numKnownSpells = numKnownSpells + 1;
        end
    end
    ...
    local buttonIndex = 1;
    for i, spellId in pairs(knownMultiCastSummonSpells) do
        -- create the button
...
function MultiCastSummonSpellButton_Update(self)
    ...
    for index, spellID in next, TOTEM_MULTI_CAST_SUMMON_SPELLS_ALT do
        if not knownMultiCastSummonSpells[index] then
            knownMultiCastSummonSpells[index] = (IsSpellKnown(spellID) and spellID) or nil;
        end
    end
...
function MultiCastSummonSpellButtonUp(id)
    if IsSpellKnown(TOTEM_MULTI_CAST_SUMMON_SPELLS[id]) then
        CastSpellByID(TOTEM_MULTI_CAST_SUMMON_SPELLS[id]);
    elseif IsSpellKnown(TOTEM_MULTI_CAST_SUMMON_SPELLS_ALT[id]) then
        CastSpellByID(TOTEM_MULTI_CAST_SUMMON_SPELLS_ALT[id]);
    end
end
...
function MultiCastRecallSpellButton_Update(self)
    ...
    for index, spellID in next, TOTEM_MULTI_CAST_RECALL_SPELLS_ALT do
        if not knownMultiCastRecallSpells[index] then
            knownMultiCastRecallSpells[index] = (IsSpellKnown(spellID) and spellID) or nil;
        end
    end
...
function MultiCastRecallSpellButtonUp(id)
    if MultiCastRecallSpellButton.spellId then
        CastSpellByID(MultiCastRecallSpellButton.spellId)
    end
end