...
function BuffFrame_Update()
	...
	VanityBuffs.pauseUpdate = true
	table.wipe(consolidatedBuffs);
	table.wipe(vanityBuffs);
	...
	BuffFrame.numVanity = #vanityBuffs;
	...
	if ( BuffFrame.numVanity > 0 ) then
		VanityBuffsCount:SetText(BuffFrame.numVanity);
		if ( not VanityBuffs:IsShown() ) then
			VanityBuffs:Show();
		end
	else
...
function AuraButton_Update(buttonName, index, filter)
	...
	if ( not name ) then
		...
		local isVanity = C_VanityCollection.IsConsolidatedVanityBuff(spellID)
		-- non vanity consolidate
		if not isVanity and ( CONSOLIDATE_BUFFS == "1" and shouldConsolidate ) then
		...
		-- vanity consolidate
		if isVanity and C_CVar.GetBool("consolidateVanityBuffs") then
			if ( buff.timeLeft and duration > 30 ) then
				buff.exitTime = expirationTime - max(10, duration / 10);
			end
			buff.expirationTime = expirationTime;
...
function AuraButton_OnClick(self, button)
	if button == "LeftButton" and IsModifiedClick("CHATLINK") then
		local spellID = select(11, UnitAura(self.unit, self:GetID(), self.filter))
		if spellID then
	...
	elseif button == "RightButton" then
		if QAToolDB and QATool and QAToolDB.RightClickDebuffs then
			QATool.RemoveAura(self:GetID(), self.filter)
		end
	end
end
...
function BuffButton_OnClick(self, button)
	if button == "RightButton" then
		...
		AuraButton_OnClick(self, button)
	end
end
...
function BuffFrame_UpdateAllBuffAnchors()
	...
	if ( BuffFrame.numConsolidated > 0 or BuffFrame.numVanity > 0 ) then
	...
	for i = 1, BUFF_ACTUAL_DISPLAY do
		...
		if ( buff.consolidated ) then
			if ( buff.parent ~= ConsolidatedBuffsContainer ) then
				buff:SetParent(ConsolidatedBuffsContainer);
				buff.parent = ConsolidatedBuffsContainer;
			end
		elseif ( buff.vanity ) then
			if ( buff.parent ~= VanityBuffsContainer ) then
			...
			elseif ( index == 1 ) then
				...
				if ( numBuffs == 1 ) then
					...
					elseif BuffFrame.numVanity > 0 then
						buff:SetPoint("TOPRIGHT", VanityBuffs, "TOPLEFT", -5, 0);
					elseif BuffFrame.numConsolidated > 0 then
	...
	if ( VanityBuffsTooltip:IsShown() ) then
...
function VanityBuffs_UpdateAllAnchors()
	local buff, previousBuff, aboveBuff;
	local numBuffs = 0;

	for _, buff in pairs(vanityBuffs) do
		numBuffs = numBuffs + 1;
		if ( buff.parent ~= VanityBuffsContainer ) then
			buff:SetParent(VanityBuffsContainer);
			buff.parent = VanityBuffsContainer;
		...
		if ( (numBuffs > 1) and (mod(numBuffs, CONSOLIDATED_BUFFS_PER_ROW) == 1) ) then
			...
			aboveBuff = buff;
		elseif ( numBuffs == 1 ) then
			buff:SetPoint("TOPLEFT", VanityBuffsContainer, "TOPLEFT", 0, 0);
			aboveBuff = buff;
		else
			buff:SetPoint("LEFT", previousBuff, "RIGHT", 7, 0);
		end
	...
	VanityBuffsTooltip:SetWidth(min(numBuffs * 24 + 18, 114));
	VanityBuffsTooltip:SetHeight(floor((numBuffs + 3) / 4 ) * CONSOLIDATED_BUFF_ROW_HEIGHT + 16);
end
...
function TemporaryEnchantFrame_OnUpdate(self, elapsed)
	...
	local hasRangedEnchant, rangedExpiration, rangedCharges
	if GetRangedWeaponEnchantInfo then
		hasRangedEnchant, rangedExpiration, rangedCharges = GetRangedWeaponEnchantInfo();
	end
	if ( not hasMainHandEnchant and not hasOffHandEnchant and not hasRangedEnchant ) then
		-- No enchants, kick out early
		TemporaryEnchantFrame_Hide();
		return;
	end

	if rangedExpiration == 0 then
		rangedExpiration = nil
	end
	...
	if ( hasRangedEnchant ) then
		...
		textureName = GetInventoryItemTexture("player", 18);
		enchantButton:SetID(18);
		_G[enchantButton:GetName().."Icon"]:SetTexture(textureName);
		enchantButton:Show();

		-- Show buff durations if necessary
		if ( rangedExpiration ) then
			rangedExpiration = rangedExpiration/1000;
		end
		AuraButton_UpdateDuration(enchantButton, rangedExpiration);

		-- Handle flashing
		if ( rangedExpiration and rangedExpiration < BUFF_WARNING_TIME ) then
	...
	if ( hasOffHandEnchant ) then
		...
		AuraButton_UpdateDuration(enchantButton, offHandExpiration);
...
function VanityBuffs_OnUpdate(self)
	-- tooltip stuff
	-- need 1-pixel outer padding because otherwise at certain resolutions OnEnter will trigger with IsMouseOver returning false
	if ( self.mousedOver and not self:IsMouseOver(1, -1, -1, 1) ) then
		self.mousedOver = nil;
		if ( not VanityBuffsTooltip:IsMouseOver() ) then
			VanityBuffsTooltip:Hide();
		end
	end

	-- check exit times
	if ( not VanityBuffs.pauseUpdate ) then
		local needUpdate = false;
		local timeNow = GetTime();
		for buffIndex, buff in pairs(vanityBuffs) do
			if ( buff.exitTime and buff.exitTime < timeNow ) then
				buff.vanity = false;
				buff.timeLeft = buff.expirationTime - timeNow;
				tremove(vanityBuffs, buffIndex);
				needUpdate = true;
			end
		end
		if ( needUpdate ) then
			if ( #vanityBuffs == 0 ) then
				BuffFrame.numConsolidated = 0;
				VanityBuffs:Hide();
			else
				BuffFrame_UpdateAllBuffAnchors();
				VanityBuffsCount:SetText(#vanityBuffs);
			end
		end
	end
end

function VanityBuffs_OnShow()
	VanityBuffsCount:SetText(BuffFrame.numVanity);
	TemporaryEnchantFrame:SetPoint("TOPRIGHT", VanityBuffs, "TOPLEFT", -6, 0);
	BuffFrame_UpdatePositions()
end

function VanityBuffs_OnEnter(self)
	VanityBuffsTooltip:SetPoint("TOPLEFT", self, "BOTTOMLEFT", 0, 0);
	-- check expiration times
	local timeNow = GetTime();
	for buffIndex, buff in pairs(consolidatedBuffs) do
		if ( buff.timeLeft ) then
	...
	VanityBuffs_UpdateAllAnchors();
	VanityBuffsTooltip:Show();
	VanityBuffs.mousedOver = true;
end

function VanityBuffs_OnHide(self)
	self.mousedOver = nil;
	VanityBuffsTooltip:Hide();
	TemporaryEnchantFrame:SetPoint("TOPRIGHT", VanityBuffs, "TOPRIGHT", 0, 0);
	BuffFrame_UpdateAllBuffAnchors();
end