...
MainMenuBarVehicleLeaveButton_OnLoad = nop
function MainMenuBarVehicleLeaveButtonMixin:OnLoad()
	...
	TaxiUtil:RegisterCallback("TAXI_STARTED", self.OnEvent, self)
	TaxiUtil:RegisterCallback("TAXI_FINISHED", self.OnEvent, self)
	-- still call this incase addons have hooked this function!
...
function MainMenuBarVehicleLeaveButtonMixin:OnEnter()
	if UnitOnTaxi("player") then
		if self:IsEnabled() == 0 then
			self.tooltipTitle = TAXI_CANCEL_PENDING
			self.tooltipText = TAXI_CANCEL_PENDING_DESCRIPTION
		else
			self.tooltipTitle = TAXI_CANCEL
			self.tooltipText = TAXI_CANCEL_DESCRIPTION
		end
	else
		self.tooltipTitle = LEAVE_VEHICLE
...
MainMenuBarVehicleLeaveButton_OnEvent = nop
function MainMenuBarVehicleLeaveButtonMixin:OnEvent(event, ...)
	self:Update()
	-- still call this incase addons have hooked this function!
	MainMenuBarVehicleLeaveButton_OnEvent(self, event, ...)
end

function MainMenuBarVehicleLeaveButtonMixin:CanExitVehicle()
	return CanExitVehicle() or TaxiUtil.CanStopTaxi()
end
...
MainMenuBarVehicleLeaveButton_Update = nop
function MainMenuBarVehicleLeaveButtonMixin:Update()
	...
	if self:CanExitVehicle() then
		...
		elseif GetNumShapeshiftForms() > 0 then
			self:SetPoint("LEFT", "ShapeshiftButton"..GetNumShapeshiftForms(), "RIGHT", 30, 0)
		else
			self:SetPoint("LEFT", PossessBarFrame, "LEFT", 10, 0)
		end
	end
	...
	-- call this incase an addon has hooked this function!
	MainMenuBarVehicleLeaveButton_Update()
end

function MainMenuBarVehicleLeaveButtonMixin:OnClick()
	if UnitOnTaxi("player") then
...
function MainMenuBarVehicleLeaveButtonMixin:OnEnable()
	self:GetNormalTexture():SetDesaturated(false);
	self:GetPushedTexture():SetDesaturated(false);
	self:UnlockHighlight();
end

function MainMenuBarVehicleLeaveButtonMixin:OnDisable()
...
function MainMenuBar_OnLoad(self)
	...
	self:HookBucketEvent("BAG_UPDATE", 0.2, GenerateClosure(MainMenuBar_OnEvent, self, "BAG_UPDATE"))
