...
function UnitFrame_Initialize (self, unit, name, portrait, healthbar, healthtext, manabar, manatext, threatIndicator, threatFeedbackUnit, threatNumericIndicator, energybar, energytext, ragebar, ragetext,
	...
	if (self.energybar) then
		self.energybar.capNumericDisplay = true;
	end
	if (self.rage) then
		self.rage.capNumericDisplay = true;
	end
	...
	UnitFrameEnergyBar_Initialize(unit, energybar, energytext, (unit == "player" or unit == "pet" or unit == "vehicle"));
	UnitFrameRageBar_Initialize(unit, ragebar, ragetext, (unit == "player" or unit == "pet" or unit == "vehicle"));
...
function UnitFrame_SetUnit (self, unit, healthbar, manabar, energybar, ragebar)
	self.unit = unit;
	healthbar.unit = unit;
	...
	if ( (self==PlayerFrame or self==PetFrame) and unit=="player") then
		...
		if ( class=="DEATHKNIGHT" or class=="HERO" or id > Enum.Class.DRUID ) then
			if ( self==PlayerFrame ) then
				RuneFrame:SetScale(1)
				RuneFrame:ClearAllPoints()
				RuneFrame:SetPoint("TOP", self,"BOTTOM", 52, 20)
...
function UnitFrame_Update (self)
	...
	UnitFrameEnergyBar_Update(self.energybar, self.unit);
	UnitFrameRageBar_Update(self.ragebar, self.unit);
...
function UnitFrame_OnEvent(self, event, ...)
	...
	elseif ( event == "UNIT_HEAL_PREDICTION" ) then
		if isUnit then
            dprint(self:GetDebugName(), "UNIT_HEAL_PREDICTION")
			UnitFrameHealPredictionBars_Update(self);
		end
	...
	elseif ( event == "UNIT_DISPLAYPOWER" ) then
		if ( isUnit and self.manabar ) then
...
--WARNING: This function is very similar to the function CompactUnitMixin.UpdateHealPrediction in CompactUnitFrame.lua.
...
function UnitFrameHealthBar_Initialize (unit, statusbar, statustext, frequentUpdates)
    ...
    statusbar:RegisterEvent("UNIT_HEAL_PREDICTION");
...
function UnitFrameEnergyBar_UnregisterDefaultEvents(self)
	self:UnregisterEvent("UNIT_ENERGY");
end

function UnitFrameEnergyBar_RegisterDefaultEvents(self)
	self:RegisterEvent("UNIT_ENERGY");
end

function UnitFrameEnergyBar_Initialize (unit, statusbar, statustext, frequentUpdates)
	if ( not statusbar ) then
		return;
	end
	statusbar.unit = unit;
	...
	if ( GetCVarBool("predictedPower") and frequentUpdates ) then
		statusbar:SetScript("OnUpdate", UnitFrameEnergyBar_OnUpdate);
	else
		UnitFrameEnergyBar_RegisterDefaultEvents(statusbar);
	end
	statusbar:RegisterEvent("UNIT_MAXENERGY");
	statusbar:SetScript("OnEvent", UnitFrameEnergyBar_OnEvent);
end

function UnitFrameEnergyBar_OnEvent(self, event, ...)
	if ( event == "CVAR_UPDATE" ) then
		TextStatusBar_OnEvent(self, event, ...);
	elseif ( event == "VARIABLES_LOADED" ) then
		self:UnregisterEvent("VARIABLES_LOADED");
		if ( GetCVarBool("predictedPower") and self.frequentUpdates ) then
			self:SetScript("OnUpdate", UnitFrameEnergyBar_OnUpdate);
			UnitFrameEnergyBar_UnregisterDefaultEvents(self);
		else
			UnitFrameEnergyBar_RegisterDefaultEvents(self);
			self:SetScript("OnUpdate", nil);
		end
	else
		UnitFrameEnergyBar_Update(self, ...);
	end
end

function UnitFrameEnergyBar_OnUpdate(self)
	if ( not self.disconnected and not self.lockValues ) then
		local currValue = UnitPower(self.unit, 3);
		if ( currValue ~= self.currValue ) then
			self:SetValue(currValue);
			self.currValue = currValue;
			TextStatusBar_UpdateTextString(self);
...
function UnitFrameEnergyBar_Update(statusbar, unit)
	if ( not statusbar or statusbar.lockValues ) then
		return;
	end
	local classMask = UnitClassMask(statusbar.unit)
	if bit.contains(Enum.ClassMask.DEFAULT, classMask) then
		statusbar:Hide()
		return
	end
	if ( unit == statusbar.unit ) then
		local maxValue = UnitPowerMax(unit, 3);
		local color = PowerBarColor["ENERGY"]
		...
		if ( statusbar.disconnected ) then
			...
			local currValue = UnitPower(unit, 3);
			statusbar:SetValue(currValue);
			statusbar.currValue = currValue;
		end
	end
...
function UnitFrameRageBar_UnregisterDefaultEvents(self)
	self:UnregisterEvent("UNIT_RAGE");
end

function UnitFrameRageBar_RegisterDefaultEvents(self)
	self:RegisterEvent("UNIT_RAGE");
end

function UnitFrameRageBar_Initialize (unit, statusbar, statustext, frequentUpdates)
	if ( not statusbar ) then
		return;
	end
	statusbar.unit = unit;
	statusbar.dontShowMax = true
	SetTextStatusBarText(statusbar, statustext);
	...
	if ( GetCVarBool("predictedPower") and frequentUpdates ) then
		statusbar:SetScript("OnUpdate", UnitFrameRageBar_OnUpdate);
	else
		UnitFrameRageBar_RegisterDefaultEvents(statusbar);
	end
	statusbar:RegisterEvent("UNIT_MAXRAGE");
	statusbar:SetScript("OnEvent", UnitFrameRageBar_OnEvent);
end

function UnitFrameRageBar_OnEvent(self, event, ...)
	if ( event == "CVAR_UPDATE" ) then
		TextStatusBar_OnEvent(self, event, ...);
	elseif ( event == "VARIABLES_LOADED" ) then
		self:UnregisterEvent("VARIABLES_LOADED");
		if ( GetCVarBool("predictedPower") and self.frequentUpdates ) then
			self:SetScript("OnUpdate", UnitFrameRageBar_OnUpdate);
			UnitFrameRageBar_UnregisterDefaultEvents(self);
		else
			UnitFrameRageBar_RegisterDefaultEvents(self);
			self:SetScript("OnUpdate", nil);
		end
	else
		UnitFrameRageBar_Update(self, ...);
	end
end

function UnitFrameRageBar_OnUpdate(self)
	if ( not self.disconnected and not self.lockValues ) then
		local currValue = UnitPower(self.unit, 1);
		if ( currValue ~= self.currValue ) then
			self:SetValue(currValue);
			self.currValue = currValue;
			TextStatusBar_UpdateTextString(self);
...
function UnitFrameRageBar_Update(statusbar, unit)
	if ( not statusbar or statusbar.lockValues ) then
		return;
	end

	local classMask = UnitClassMask(unit)
	if not classMask or bit.contains(Enum.ClassMask.DEFAULT, classMask) then
		statusbar:Hide()
		return
	end

	if ( unit == statusbar.unit ) then
		local maxValue = UnitPowerMax(unit, 1);
		local color = PowerBarColor["RAGE"]
		statusbar:SetStatusBarColor(color.r, color.g, color.b)
		statusbar:SetMinMaxValues(0, maxValue);
		...
		if ( statusbar.disconnected ) then
			...
			local currValue = UnitPower(unit, 1);
			statusbar:SetValue(currValue);
			statusbar.currValue = currValue;
		end
	end
