...
QUEST_TEMPLATE_OVERRIDES = {}
--[[local json = LoadAscensionContentJSON("Quest")
if ( json and json ~= '' ) then
	json = C_Serialize:FromJSON(json)
	for _, d in ipairs(json) do
		QUEST_TEMPLATE_OVERRIDES[d.QuestId] = { ["HideObjectivesTextSlain"] = toboolean(d.HideObjectivesTextSlain) };
	end
end]]

function QuestInfo_GetObjectivesText(questId, text)
	if ( QUEST_TEMPLATE_OVERRIDES[questId] ~= nil and QUEST_TEMPLATE_OVERRIDES[questId]["HideObjectivesTextSlain"] == true ) then
		text = gsub(text, " slain", "");
	end

	return text;
end

function QuestInfo_IsAutoQuestWatch(questId)
	if ( QUEST_TEMPLATE_OVERRIDES[questId] ~= nil and QUEST_TEMPLATE_OVERRIDES[questId]["AutoQuestWatch"] == true ) then
		return true;
	end

	return AUTO_QUEST_WATCH == "1";
end

function QuestInfoFadingFrame_OnUpdate(self, elapsed)
	if ( self.fading ) then
		if self.fadingProgress >= 0 then
...
function QuestInfoItem_OnClick(self)
	if ( self.type == "choice" ) then
		...
		EventRegistry:TriggerEvent("Quest.QuestRewardItemOnClick")
	end
end
...
function QuestInfo_ShowFadingFrame()
	...
	if ( QUEST_FADING_DISABLE == "1" ) then
		QuestInfoDescriptionText:SetAlphaGradient(0, 0);
		QuestInfoFadingFrame.fadingProgress = -1;
	else
