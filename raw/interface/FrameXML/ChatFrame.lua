...
-- Timer ticker reference for scrolling the chat to the bottom after 60 seconds
local chatTicker
...
SecureCmdList["CAST"] = function(msg)
	...
	if ( action ) then
		...
		-- hack to handle [@player] or [target=player] macros
		local castAtPlayer = target and target:lower() == "player"
		...
		if castAtCursor then
			local originalTarget, guid = UnitName("target"), UnitGUID("target")
			Custom_HandleTerrainClick()
			local newTarget, newTargetGuid = UnitName("target"), UnitGUID("target")

			if originalTarget and guid ~= newTargetGuid then
				TargetLastTarget()
			elseif not originalTarget and newTarget ~= nil then
				ClearTarget()
			end
		elseif castAtPlayer then
			local originalTarget, guid = UnitName("target"), UnitGUID("target")
...
local delayCommands = {}
local indelayEditBox = CreateFrame("EditBox", "ChatInDelayEditBox")
indelayEditBox:SetAutoFocus(false)
indelayEditBox:ClearFocus()

SlashCmdList["ASCENSIONDATABASE"] = function(msg)
	local location = msg:match("^(%S+)")
	...
	if location == DATABASEURL_ITEM then
		if id then
			OpenAscensionDBURL("?item="..id)
		else
			OpenAscensionDBURL("?items")
		end
	elseif location == DATABASEURL_SPELL then
		if id then
			OpenAscensionDBURL("?spell="..id)
		else
			OpenAscensionDBURL("?spells")
		end
	elseif location == DATABASEURL_ACHIEVEMENT then
		if id then
			OpenAscensionDBURL("?achievement="..id)
		else
			OpenAscensionDBURL("?achievements")
		end
	elseif location == DATABASEURL_NPC then
		if id then
			OpenAscensionDBURL("?npc="..id)
		else
			OpenAscensionDBURL("?npcs")
		end
	else
		SendSystemMessage(ASCENSIONDATABASE_USAGE)
	end
end

SlashCmdList["ASCENSIONWEB"] = function(msg)
	OpenAscensionURL("")
end

SlashCmdList["ASCENSIONSHOP"] = function(msg)
	OpenAscensionURL("shop")
end

SlashCmdList["ASCENSIONWIKI"] = function(msg)
	if string.isNilOrEmpty(msg) then
		OpenAscensionWikiURL("wiki")
		return
	end
	OpenAscensionWikiURL("wiki/Special:Search?query="..msg:gsub("%s", "+").."&scope=internal&navigationSearch=true")
end

SlashCmdList["WCROLL"] = function()
	WildCardUtil.ShowRollDiceFromSlashCommand()
end
SLASH_WCROLL1 = "/wcroll"

SlashCmdList["ENCHANTSPEC"] = function(msg)
	local presetID = msg and tonumber(msg)
	if presetID and presetID <= C_MysticEnchantPreset.GetNumPresets() and presetID > 0 then
		MysticEnchantManagerUtil.AttemptOperation("Activate", "CanActivate", presetID)
	end
end

SlashCmdList["CHALLENGES"] = function()
	ToggleChallengesUI()
end

SlashCmdList["BUGREPORT"] = function()
...
SlashCmdList["VERSION"] = function(msg)
	if msg ~= "" then
		return C_TrinityCore:RequestPlayerVersion(msg)
	end
	SendSystemMessage(format("%s: %s@%s %s", GAME_VERSION_LABEL, GetClientVersion()))
end

SLASH_VERSION1 = "/version"

SlashCmdList["LOGDEBUG"] = function()
	if GetCVarBool("logDebug") then
		SetCVar("logDebug", "0")
		print("Disabled debug logging!")
	else
		SetCVar("logDebug", "1")
		print("Enabled debug logging! Type `/logdebug` again to disable.")
	end
end

SLASH_LOGDEBUG1 = "/logdebug"

SlashCmdList["LOGINFO"] = function()
	if GetCVarBool("logInfo") then
		SetCVar("logInfo", "0")
		print("Disabled info logging!")
	else
		SetCVar("logInfo", "1")
		print("Enabled info logging! Type `/loginfo` again to disable.")
	end
end

SLASH_LOGINFO1 = "/loginfo"

SlashCmdList["INDELAY"] = function(msg)
	local delay, name, command, args = msg:match("^(%S+)%s+%[?([^%]^%s]*)%]?%s*(/%S+)(.*)")
	...
	if not delay then
		delay, name = msg:match("^(%S+)%s+%[?([^%]]*)%]?")
		if delay and delay:lower() == CANCEL:lower() and name then
			name = name:lower()
			if delayCommands[name] then
				delayCommands[name]:Cancel()
				delayCommands[name] = nil
				ChatFrame_DisplayUsageError(format(CHAT_INDELAY_STOPPED, name))
			end
			return
		end
		ChatFrame_DisplayUsageError(CHAT_INDELAY_USAGE)
		return
	end

 	delay = tonumber(delay)
	if not delay or delay <= 0 or not name and not command then
	    ChatFrame_DisplayUsageError(CHAT_INDELAY_USAGE)
	    return
	end

	if IsSecureCmd(command) then
		ChatFrame_DisplayUsageError(format(CHAT_INDELAY_SECURE, command))
		return
	end
	
	args = args or ""

	if name and delayCommands[name] then
		delayCommands[name]:Cancel()
		delayCommands[name] = nil
		ChatFrame_DisplayUsageError(format(CHAT_INDELAY_UPDATED, name, delay, command, args))
	end

	local timer = Timer.NewTimer(delay, function()
		if name then
			delayCommands[name] = nil
		end
		indelayEditBox:SetText(command..args)
		ChatEdit_SendText(indelayEditBox)
	end)

	if name then
		delayCommands[name] = timer
	end
end
...
SLASH_MEASURE1 = "/measure"
local measureOrigin
SlashCmdList["MEASURE"] = function()
	local x, y, z, mapID = GetCurrentPlayerPosition()
	if measureOrigin and measureOrigin.mapID == mapID then
		local distance = sqrt((x - measureOrigin.x) ^ 2 + (y - measureOrigin.y) ^ 2 + (z - measureOrigin.z) ^ 2)
		print(format("Distance: %.2f yards", distance))
	else
		measureOrigin = { x = x, y = y, z = z, mapID = mapID }
		print("Type /measure again to measure the distance from this point.")
	end
end
...
SlashCmdList["INVITE"] = function(msg)
	...
	if C_Player:IsInGroup() and not C_Player:IsGroupLeader() then
		SuggestInvite(msg)
	else
		InviteUnit(msg);
	end
...
SLASH_LUACONSOLE1 = "/luaconsole"
SLASH_LUACONSOLE2 = "/devconsole"
SlashCmdList["LUACONSOLE"] = function()
	if not IsAddOnLoaded("Ascension_UIDevelopmentTools") then
		local loaded = LoadAddOn("Ascension_UIDevelopmentTools")

		if not loaded then
			SendSystemMessage("Ascension UI Development Tools are not enabled. You can enable this in the AddOns menu.")
			return
		end
	end

	if DevConsole:IsShown() then
		DevConsole:Close()
	else
		DevConsole:Show()
	end
...
SLASH_ATLASBROWSER1 = "/atlasbrowser"
SLASH_ATLASBROWSER2 = "/ab"
SLASH_ATLASBROWSER3 = "/av" -- from old version
SlashCmdList["ATLASBROWSER"] = function()
	if not IsAddOnLoaded("Ascension_UIDevelopmentTools") then
		local loaded = LoadAddOn("Ascension_UIDevelopmentTools")
...
SlashCmdList["EVENTTRACE"] = function(msg)
	...
	if AscensionEventTrace:IsShown() and msg == "" then
		AscensionEventTrace:Hide()
	else
		...
		elseif msg:lower() == "stop" or msg:lower() == "pause" then
			if not EventTrace.IsPaused() then
				EventTrace.Stop()
			end
		end
	end
...
SLASH_TABLEINSPECT2 = "/ti"
SLASH_TABLEINSPECT3 = "/tdump"
SlashCmdList["TABLEINSPECT"] = function(msg)
	...
	TableInspectorFrame:Show()
	msg = msg:trim()
	if not string.isNilOrEmpty(msg) then
		TableInspectorFrame.Search:SetText(msg)
		TableInspectorFrame:SearchObject(msg)
	end
end

SLASH_OLDEVENTTRACE1 = "/oldeventtrace"
SLASH_OLDEVENTTRACE2 = "/oldeventtrace"
SLASH_OLDEVENTTRACE3 = "/oldetrace"
SLASH_OLDEVENTTRACE4 = "/oldetrace"
SlashCmdList["OLDEVENTTRACE"] = function(msg)
	UIParentLoadAddOn("Blizzard_DebugTools");
	EventTraceFrame_HandleSlashCmd(msg);
end
...
SlashCmdList["GLYPH"] = function(msg)
	ToggleGlyphFrame()
end

SlashCmdList["JSON"] = function(msg)
	JSON_BENCHMARKS:Print(msg)
end

SlashCmdList["TARGETRANGE"] = function(msg)
	local value = tonumber(msg)
	if value then
		value = math.clamp(value, 5, 100)
		C_CVar.Set("tabTargetRange", value)
	end
end

SlashCmdList["TARGETFACING"] = function(msg)
	local value = toboolean(msg)
	C_CVar.Set("tabTargetFacing", value and "1" or "0")
end

SLASH_WATCHFUNC1 = "/dfunc"
SlashCmdList["WATCHFUNC"] = function(msg)
	if msg and strlen(msg) > 0 then
		dfunc(msg)
	end
end

SLASH_TRACEFUNC1 = "/tracefunc"
SLASH_TRACEFUNC2 = "/tfunc"
SlashCmdList["TRACEFUNC"] = function(msg)
    if msg and strlen(msg) > 0 then
        tfunc(msg)
    end
end
...
function ChatFrame_OnLoad(self)
	...
	tinsert(CHAT_FRAMES, self);
end

function ChatFrame_EnumerateChatFrames()
	local index = 1;
	return function()
		local frame = CHAT_FRAMES[index]
		index = index + 1;
		return frame;
	end
end
...
function ChatFrame_ConfigEventHandler(self, event, ...)
	...
	if ( event == "UPDATE_CHAT_COLOR" ) then
		...
		if ( info ) then
			...
			-- Replace default bg leader with retail color
			if strupper(arg1) == "BATTLEGROUND_LEADER" then
				if math.NearlyEquals(arg2, 1.000000059139) and math.NearlyEquals(arg3, 0.85882358020172) and math.NearlyEquals(arg4, 0.71764710126445) then
					info.r = 1
					info.g = 0.282352941176
					info.b = 0.035294117647
				end
			end
		end
		return true;
	...
	elseif ( event == "VARIABLES_LOADED" ) then
		...
		local value = GetCVar("showTimestamps");
		if ( value == "none" ) then
...
function ChatFrame_FormatURL(url)
...
function ChatFrame_MessageEventHandler(self, event, ...)
	if ( strsub(event, 1, 8) == "CHAT_MSG" ) then
		...
		-- @robinsch: support to add chat checks (chatFilters) from server
		if ( GetChatCheck ~= nil and GetChatCheck(event, arg1) ) then
			return true;
		end

		for _, pattern in pairs(URL_PATTERNS) do
			if string.find(arg1, pattern) then
				arg1 = string.gsub(arg1, pattern, ChatFrame_FormatURL("%1"))
				break
			end
		end
...
function ChatFrame_OnUpdate(self, elapsedSec)
	...
	if currentScroll ~= self:GetCurrentScroll() then
		currentScroll = self:GetCurrentScroll()
		if chatTicker then
			chatTicker:Cancel()
			chatTicker = nil
		end
	...
	if not chatTicker then
		chatTicker = Timer.NewTicker(60, function()
			self:ScrollToBottom()
			chatTicker = nil
		end, 1)
	end
...
function ChatFrame_SendTell(name, chatFrame)
	...
	if not editBox then return end --? @andrew
...
function ChatEdit_SetInsertTarget(target)
	local targetName
	...
	UIParent:SetAttribute("chatInsertTarget", targetName)
end

function ChatEdit_ClearInsertTarget(target)
	if target and ChatEdit_GetInsertTarget() ~= target then
		return
	end
	UIParent:SetAttribute("chatInsertTarget", nil);
end

function ChatEdit_GetInsertTarget()
	return UIParent:GetAttribute("chatInsertTarget")
end

function ChatEdit_InsertLinkToTarget(text)
	local target = ChatEdit_GetInsertTarget()
	if _G[target] then
		securecall(_G[target].Insert, _G[target], text);
	end
end
...
function ChatEdit_InsertLink(text)
	...
	if TicketFrame and TicketFrame:InsertLink(text) then
		return true;
	end
	...
	if ChatEdit_GetInsertTarget() ~= nil then
		securecall(ChatEdit_InsertLinkToTarget, text);
		return true
	end
	return false;
end
...
function SendLootMessage(itemLink, quantity)
	if not issecure() then return end
	local msg = quantity and quantity > 1 and LOOT_ITEM_PUSHED_SELF_MULTIPLE or LOOT_ITEM_PUSHED_SELF
	msg = msg:format(itemLink, quantity)
	for chatFrame in ChatFrame_EnumerateChatFrames() do
		if chatFrame:IsEventRegistered("CHAT_MSG_LOOT") then
			ChatFrame_OnEvent(chatFrame, "CHAT_MSG_LOOT", msg, "", "", "", "", "", 0, 0, "", 0, 0, "", 0)
		end
	end
end
...
--
ChatFrame_AddMessageEventFilter("CHAT_MSG_CHANNEL_NOTICE_USER", function(self, event, arg1, arg2, arg3, arg4, arg5, arg6, arg7, arg8, arg9)
	-- this msg is sent by server trying to force you into a channel
	if arg1 == "PLAYER_ALREADY_MEMBER" and (arg9:lower() == "newcomers" or arg9:lower() == "ascension") then
		return true
	end
end)
...
-- Chat Infraction
--
ChatInfractionFrameMixin = CreateFromMixins("NineSlicePanelMixin")

function ChatInfractionFrameMixin:OnShow()
	self.AcknowledgeButton:Disable()
	self.AcknowledgeEditBox:SetText("")
	
	self.AcknowledgeEditBox.Label:SetFormattedText(TYPE_S_BELOW, HIGHLIGHT_FONT_COLOR:WrapText(I_WILL_FOLLOW_THE_CODE_OF_CONDUCT))
end 

function ChatInfractionFrameMixin:OnTextChanged(text)
	if text == I_WILL_FOLLOW_THE_CODE_OF_CONDUCT then
		self.AcknowledgeButton:Enable()
	else
		self.AcknowledgeButton:Disable()
	end
end 
