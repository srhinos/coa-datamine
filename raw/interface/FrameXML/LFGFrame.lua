...
function LFGDebug(text, ...)
    if C_Logger.LFG then
        C_Logger.LFG("LFGLua: "..format(text, ...))
    end
...
function LFG_UpdateLockedOutPanels()
	...
	if ( mode == "listed" ) then
		...
		if lfd then
			lfd.NoLFDWhileLFR:Show()
		end
	else
		LFDQueueFrameNoLFDWhileLFR:Hide();
		if lfd then
			lfd.NoLFDWhileLFR:Hide()
		end
	end
...
function LFG_UpdateQueuedList()
	...
	if ( mode == "queued" or mode == nil ) then
...
function LFG_UpdateAvailableRoles()
	...
	if ( canBeTank ) then
		...
		if lfd then
			LFG_PermanentlyDisableRoleButton(lfd.Tank)
		end
	end
	...
	if ( canBeHealer ) then
		...
		if lfd then
			LFG_EnableRoleButton(lfd.Healer)
		end
	else
		...
		if lfd then
			LFG_PermanentlyDisableRoleButton(lfd.Healer)
		end
	end
	...
	if ( canBeDPS ) then
		...
		if lfd then
			LFG_PermanentlyDisableRoleButton(lfd.Dps)
		end
	end
	...
	if ( canChangeLeader ) then
		...
		if lfd then
			LFG_EnableRoleButton(lfd.Leader)
		end
	else
		LFG_PermanentlyDisableRoleButton(LFDQueueFrameRoleButtonLeader);
		if lfd then
			LFG_PermanentlyDisableRoleButton(lfd.Leader)
		end
	end
end
...
function LFG_UpdateRoleCheckboxes()
	...
	if _G["AscensionPVEFrame"] then
		local lfd = _G["AscensionPVEFrameLFDFrame"]
		lfd.Leader.checkButton:SetChecked(leader)
		lfd.Tank.checkButton:SetChecked(tank)
		lfd.Healer.checkButton:SetChecked(healer)
		lfd.Dps.checkButton:SetChecked(dps)
	end
end
...
function LFG_UpdateRolesChangeable()
	...
	if ( mode == "queued" or mode == "listed" or mode == "rolecheck" or mode == "proposal" ) then
		...
		if _G["AscensionPVEFrame"] then
			local lfd = _G["AscensionPVEFrameLFDFrame"]
			LFG_DisableRoleButton(lfd.Tank, true)
			LFG_DisableRoleButton(lfd.Healer, true)
			LFG_DisableRoleButton(lfd.Dps, true)
			LFG_DisableRoleButton(lfd.Leader, true)
		end
	else
...
function LFGDungeonList_Setup(force)
	if not hasSetUp or force then
		hasSetUp = true;
