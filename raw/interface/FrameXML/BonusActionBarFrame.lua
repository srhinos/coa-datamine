...
function ShapeshiftBar_UpdateState ()
	...
	for i=1, NUM_SHAPESHIFT_SLOTS do
		...
		if ( i <= numForms ) then
			...
			button:Show();
			ActionButton_UpdateHotkeys (button, "SHAPESHIFTBUTTON")
		else
			button:Hide();
		end
	end
    ...
    if MultiCastActionBarFrame and MultiCastActionBarFrame:IsShown() then
        MULTICASTACTIONBAR_XPOS = 50 + ((ShapeshiftBarFrame:GetWidth() + 8) * numForms)
...
function ShapeshiftButton_OnLoad(self)
	self.buttonType = "SHAPESHIFTBUTTON"
	MixinAndLoad(self, "QuickKeybindButtonMixin")
end

function ShapeshiftButton_OnClick(self, button)
	if self:QuickKeybindButtonOnClick(button) then
		return
	end
...
function ShapeshiftButton_OnEnter(self)
...
function ShapeshiftButton_OnLeave(self)
