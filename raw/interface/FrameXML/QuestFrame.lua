...
function QuestFrame_OnEvent(self, event, ...)
	...
	if event == "QUEST_TURNED_IN" then
		RunNextFrame(SuperTrackerUtil.UpdateSelectedQuest)
		return
	end
...
function QuestFrame_ShowQuestPortrait(parentFrame, portrait, text, name, x, y, questID, forceDisplayWatchFrame)
    ...
    QuestNPCModel:SetFrameLevel(100)
    QuestNPCModelTextFrame:SetFrameLevel(QuestNPCModel:GetFrameLevel()-1)
    QuestNPCModelCloseButton:Hide()
    QuestNPCModel.forceDisplayWatchFrame = forceDisplayWatchFrame
    QuestNPCModel.questID = questID
    QuestNPCModel.portrait = portrait
    ...
    if (portrait == -1) then
        QuestNPCModel:SetCamera(0);
        QuestNPCModel:SetUnit("player");
        QuestNPCModel:RefreshUnit();
    else
        QuestNPCModel:SetDisplayInfo(portrait);
        QuestNPCModel:SetCamera(1);
    end

    Timer.After(0.1, function() -- bugged camera fix
    	QuestNPCModel:Show();
    	QuestNPCModel:SetDisplayInfo(QuestNPCModel.portrait)
    end)
end
...
function QuestFrameRewardPanel_OnShow()
	...
	local questPortrait, questPortraitText, questPortraitName = C_Quest:GetPortraitData();
	if (questPortrait) then
		QuestFrame_ShowQuestPortrait(QuestFrame, questPortrait, questPortraitText, questPortraitName, -32, -62);
	else
...
function QuestFrameDetailPanel_OnShow()
	...
	local questPortrait, questPortraitText, questPortraitName, forceDisplayWatchFrame, questID = C_Quest:GetPortraitData();
	if (questPortrait) then
		QuestFrame_ShowQuestPortrait(QuestFrame, questPortrait, questPortraitText, questPortraitName, -32, -62, questID, forceDisplayWatchFrame);
	else
		QuestFrame_HideQuestPortrait();
	end
end
