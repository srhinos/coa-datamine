...
function QuestLogTitleButton_OnEnter(self)
	...
	local link = GetQuestLink(self:GetID())
...
function QuestLog_OnEvent(self, event, ...)
	...
	elseif ( event == "QUEST_ACCEPTED" ) then
		-- @robinsch: quest auto watch (tracking) override
		local questId = select(9, GetQuestLogTitle(arg1));
		if ( QuestInfo_IsAutoQuestWatch(questId) and GetNumQuestWatches() < MAX_WATCHABLE_QUESTS ) then
			AddQuestWatch(arg1);
			QuestLog_Update();
		end

		if self:IsVisible() then
			QuestLog_SetSelection(arg1);
...
function QuestLog_UpdateTrackButton()
	local selectedQuest = GetQuestLogSelection()
	if selectedQuest and IsQuestWatched(selectedQuest) then
		QuestLogFrameTrackButton:SetText(UNTRACK_QUEST_ABBREV)
	else
...
function QuestLog_Update()
	...
	for i=1, numButtons do
		...
		if ( questIndex <= numEntries ) then
			...
			elseif GetQuestScaling(questID) then
...
function QuestLog_UpdatePortrait()
	if not(QuestLogFrame) or not(QuestLogFrame.selectedIndex) then
		return
	end

	if not(C_Quest:GetQuestID(QuestLogFrame.selectedIndex)) then
		return
	end

	local questPortrait, questPortraitText, questPortraitName = C_Quest:GetPortraitData(C_Quest:GetQuestID(QuestLogFrame.selectedIndex));
