...
function PlayerFrame_OnLoad(self)
	UnitFrame_Initialize(self, "player", PlayerName, PlayerPortrait,
						 ...
						 PlayerFrameEnergyBar, PlayerFrameEnergyBarText,
						 PlayerFrameRageBar, PlayerFrameRageBarText,
...
function PlayerFrame_UpdatePrimaryStat()
	local stat = C_PrimaryStat:GetActivePrimaryStat()
	if not stat or UnitHasVehicleUI("player") then
		if PlayerPrimaryStat:IsMouseOver() then
			PlayerPrimaryStat:CallScript("OnLeave")
		end
		PlayerPrimaryStat:Hide()
		return
	end

	PlayerPrimaryStat:Show()
	local spellID, _, icon = C_PrimaryStat:GetPrimaryStatInfo(stat)
	local tooltip = "spell:"..spellID
	PlayerPrimaryStat.tooltip = tooltip
	PlayerPrimaryStat.Icon:SetPortraitTexture(icon)
	if PlayerPrimaryStat:IsMouseOver() then
		PlayerPrimaryStat:CallScript("OnEnter")
	end
end
...
function PlayerFrame_OnEvent(self, event, ...)
	...
	elseif ( event == "PLAYER_ROLES_ASSIGNED" ) then
		PlayerFrame_UpdateRolesAssigned();
	elseif ( event == "CHARACTER_ADVANCEMENT_PENDING_BUILD_UPDATED" ) then
		PlayerFrame_UpdatePrimaryStat()
	end
end
...
function PlayerFrame_ToVehicleArt(self, vehicleType)
	...
	UnitFrame_SetUnit(self, "vehicle", PlayerFrameHealthBar, PlayerFrameManaBar, PlayerFrameEnergyBar, PlayerFrameRageBar);
	...
	if ( vehicleType == "Natural" ) then
		...
		PlayerFrameHealthBar:SetWidth(104);
		PlayerFrameHealthBar:SetPoint("TOPLEFT",116,-41);
		PlayerFrameManaBar:SetWidth(104);
...
function PlayerFrame_ToPlayerArt(self)
	...
	UnitFrame_SetUnit(self, "player", PlayerFrameHealthBar, PlayerFrameManaBar, PlayerFrameEnergyBar, PlayerFrameRageBar);
	...
	if class == "HERO" then
		local artwork = C_CVar.GetBool("showPortraitDragon") and "Interface\\TargetingFrame\\UI-HeroFrame-Dragon" or "Interface\\TargetingFrame\\UI-HeroFrame"
		PlayerFrameTexture:SetTexture(artwork)
		PlayerFrameEnergyBar:Show()
		PlayerFrameEnergyBar.pauseUpdates = nil
		PlayerFrameEnergyBarText:Show()
		PlayerFrameRageBar:Show()
		PlayerFrameRageBar.pauseUpdates = nil
		PlayerFrameRageBarText:Show()
		PlayerFrameTexture:SetPoint("TOPLEFT", 8, 0)
		PlayerFrameTexture:SetPoint("BOTTOMRIGHT", 8, 0)
	else
		local artwork = C_CVar.GetBool("showPortraitDragon") and "Interface\\TargetingFrame\\UI-TargetingFrame-Rare-Elite" or "Interface\\TargetingFrame\\UI-TargetingFrame"
		PlayerFrameTexture:SetTexture(artwork)
		PlayerFrameEnergyBar:Hide()
		PlayerFrameEnergyBar.pauseUpdates = true
		PlayerFrameEnergyBarText:Hide()
		PlayerFrameRageBar:Hide()
		PlayerFrameRageBar.pauseUpdates = true
		PlayerFrameRageBarText:Hide()
		PlayerFrameTexture:SetPoint("TOPLEFT", 0, 0)
		PlayerFrameTexture:SetPoint("BOTTOMRIGHT", 0, 0)
	end
	...
	PlayerFrameHealthBar:SetWidth(116);
	PlayerFrameHealthBar:SetPoint("TOPLEFT",106,-41);
	PlayerFrameManaBar:SetWidth(116);
	PlayerFrameManaBar:SetPoint("TOPLEFT",106,-52);
	PlayerFrameBackground:SetWidth(116);
