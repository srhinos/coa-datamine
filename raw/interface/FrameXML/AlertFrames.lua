...
local FILTER_CATEGORY = {
	[16135] = true, -- Ascension Seasonal Parent Tab
	[15105] = true, -- Former Season Achievements
	[16133] = true, -- Current Season Achievements
}
...
function AlertFrame_OnEvent (self, event, ...)
	if ( event == "ACHIEVEMENT_EARNED" ) then
		...
		if ( not ShouldShowAchievementToast(id) ) then
			return;
		end
		...
		if category and FILTER_CATEGORY[category] then
			return
		end

		C_PopupQueue:AddAchievement(id)
