...
MAX_GUILDMEMBERS_PROCESSED = 500;
GUILDMEMBERS_PROCESS_DELAY = 0.1;
...
function FriendsFrame_OnShow()
	...
	SetGuildRosterSelection(0);
	InGuildCheck();
	Timer.After(1, FriendsList_Update)
end
...
local QueuedRosterUpdates = {}
local OnlineCount = 0

local function FinalizeRosterUpdate(numGuildMembers)
	local button, buttonText, guildIndex, classTextColor
...
local function ProcessRosterMembers(startIndex, endIndex)
	for i = startIndex, startIndex + endIndex do
...
function GuildStatus_Update()
	...
	-- Cancel previous roster update.
	for i, timer in pairs(QueuedRosterUpdates) do
	...
	local membersToProcess = numGuildMembers
	local delay = GUILDMEMBERS_PROCESS_DELAY
	local startIndex = numGuildMembers - membersToProcess
	local endIndex = min(numGuildMembers, startIndex + MAX_GUILDMEMBERS_PROCESSED)
	-- Quickly run through the first MAX_GUILDMEMBERS_PROCESSED members of the guild like normal
	ProcessRosterMembers(startIndex, endIndex)
	membersToProcess = max(membersToProcess - MAX_GUILDMEMBERS_PROCESSED, 0)

	local didDelay = false
	-- Delay GetRosterInfo calls by 0.01 (10ms) to try and avoid hangs in large guilds
	while membersToProcess > 0 do
		didDelay = true
		startIndex = numGuildMembers - membersToProcess
		endIndex = min(numGuildMembers, startIndex + MAX_GUILDMEMBERS_PROCESSED)

		local timer = Timer.NewTimer(delay, function(self)
			ProcessRosterMembers(self.startIndex, self.endIndex)
		end)
		timer.startIndex = startIndex
		timer.endIndex = endIndex

		delay = delay + GUILDMEMBERS_PROCESS_DELAY
		membersToProcess = max(membersToProcess - MAX_GUILDMEMBERS_PROCESSED, 0)
		tinsert(QueuedRosterUpdates, timer)
	end
	...
	FinalizeRosterUpdate(numGuildMembers)
end
