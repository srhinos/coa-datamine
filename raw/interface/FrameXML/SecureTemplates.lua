...
local SECURE_REPEAT_FRAME = CreateFrame("Frame")
SECURE_REPEAT_FRAME.Actions = {}
SECURE_REPEAT_FRAME:SetScript("OnUpdate", function(self, elapsed)
	for button in pairs(self.Actions) do
		SecureActionButton_OnUpdate(button, elapsed)
	end
end)
...
function SecureButton_GetModifiedUnit(self, button)
	...
	if ( SecureButton_GetModifiedAttribute(self, "checkmouseovercast", button) ) then
		local mouseoverMod = GetModifiedClick("MOUSEOVERCAST");
		local useMouseoverCasting = C_CVar.GetBool("enableMouseoverCast") and
			(mouseoverMod == "NONE" or
			(mouseoverMod == "SHIFT" and IsShiftKeyDown()) or
			(mouseoverMod == "CTRL" and IsControlKeyDown()) or
			(mouseoverMod == "ALT" and IsAltKeyDown()));
		if ( useMouseoverCasting and UnitExists("mouseover") ) then
			local targetIsFriendly = UnitCanAssist("player", "mouseover");
			local spellID = SecureActionButton_GetActionSpell(self, button)
			if spellID then
				if targetIsFriendly and C_Spell.IsHelpfulSpell(spellID, true) or C_CVar.GetBool("enableMouseoverCastHarm") and not targetIsFriendly and C_Spell.IsHarmfulSpell(spellID, true) then
...
SECURE_ACTIONS.spellbook =
function (self, unit, button)
	local spell = SecureButton_GetModifiedAttribute(self, "spell", button);
	local bookType = SecureButton_GetModifiedAttribute(self, "bookType", button);
	local spellID = tonumber(spell);
	if bookType then
		if bookType == BOOKTYPE_PET and button ~= "LeftButton" then
			ToggleSpellAutocast(spell, bookType)
		end
		CastSpell(spell, bookType);
	elseif ( spellID) then
		CastSpellByID(spellID, unit);
	elseif ( spell ) then
		CastSpellByName(spell, unit);
...
function (self, unit, button)
	...
	if ( index ) then
		...
		if type(spellID) == "number" then
			for i = 1, 40 do
				local auraID = select(11, UnitBuff(unit, i))
			...
			CancelUnitBuff(unit, spell, rank or "");
		end
	end
end;
...
function SecureActionButton_OnClick(self, button, down)
	if self.QuickKeybindButtonOnClick then
		if self:QuickKeybindButtonOnClick(button, down) then
			return
		end
	end
	-- TODO check with Tom etc if this is kosher
	if (down) then
		if self.IsHoldCast and IsModifiedClick("PICKUPACTION") then
			return
		end
	...
	-- if we are coming from pushed and we have a repeat action then just exit here
	-- button state will be whatever state the button is in BEFORE the click is finalized.
	if self:GetButtonState() == "PUSHED" and self.RepeatAction then
		self.RepeatAction = nil
		SECURE_REPEAT_FRAME.Actions[self] = nil
		return
	end
	...
	if ( actionType ) then
		...
		if not handler then
			...
			if self.IsHoldCast and self:GetButtonState() == "NORMAL" then
				self.RepeatAction = self.RepeatAction or GenerateClosure(handler, self, unit, button, actionType)
				SECURE_REPEAT_FRAME.Actions[self] = true
			end
		end
		if ( not handler ) then
...
function SecureActionButton_OnLoad(self)
	-- redirect re-registering this to hook instead
	self:SetScript("OnAttributeChanged", function(self, name, value)
		if self._isActionBarButton then
			ActionButton_UpdateAction(self, name, value);
		end
		SecureActionButton_OnAttributeChanged(self, name, value)
	end)

	self._SetScript = self.SetScript
	function self:SetScript(handler, ...)
		if handler == "OnAttributeChanged" then
			self:HookScript(handler, ...)
		else
			self:_SetScript(handler, ...)
		end
	end
	
	self._RegisterForClicks = self.RegisterForClicks
	function self:RegisterForClicks(...)
		self.DesiredClickButtons = { ... }
		if self.IsHoldCast then
			return
		end
		
		self:_RegisterForClicks(...)
	end
	
	C_Hook:Register(self, "CVAR_UPDATE", function(cvar)
		if cvar == "HOLD_TO_CAST_TEXT" then
			SecureActionButton_CheckHoldCastState(self)
		end
	end)

	if C_CVar.GetBool("holdToCast") then
		SecureActionButton_CheckHoldCastState(self)
	end
end

function SecureActionButton_GetActionSpell(self)
	local button1Action = SecureButton_GetModifiedAttribute(self, "type", "LeftButton")
	local spellID

	if button1Action == "action" then
		local action = ActionButton_CalculateAction(self, "LeftButton")
		if action then
			local actionType, macroID, _, actionSpellID = GetActionInfo(action)
			if actionType == "macro" then
				local spellName, spellRank = GetMacroSpell(macroID)
				if spellName then
					spellID = C_Spell:GetSpellID(spellName, spellRank)
				end
			elseif actionType == "spell" then
				spellID = actionSpellID
			end
		end
	elseif button1Action == "spell" then
		local spell = SecureButton_GetModifiedAttribute(self, "spell", "LeftButton")
		spellID = tonumber(spell) or C_Spell:GetSpellID(spell)
	end

	return spellID
end

function SecureActionButton_CheckHoldCastState(self)
	if not self._RegisterForClicks then return end
	local clicks
	
	local spellID = SecureActionButton_GetActionSpell(self)

	if spellID and C_Spell:ShouldHoldToCast(spellID) then
		clicks = { "AnyUp", "AnyDown" }
		self.IsHoldCast = true
	else
		self.IsHoldCast = false
	end

	if clicks then
		self:_RegisterForClicks(unpack(clicks))
	elseif self.DesiredClickButtons then
		self:_RegisterForClicks(unpack(self.DesiredClickButtons))
	else
		self:_RegisterForClicks("AnyUp")
	end
end

function SecureActionButton_OnAttributeChanged(self, attribute, value)
	if not attribute:find("type") then return end
	RunNextFrame(function() SecureActionButton_CheckHoldCastState(self) end)
end

local RepeatThrottle = 0.05
function SecureActionButton_OnUpdate(self, elapsed)
	if IsModifiedClick("PICKUPACTION") then return end
	if GetCursorInfo() ~= nil then return end

	if not self.RepeatAction or not self.IsHoldCast or self:GetButtonState() ~= "PUSHED" then
		self.RepeatAction = nil
		SECURE_REPEAT_FRAME.Actions[self] = nil
		return
	end
	...
	if self.updateRate and self.updateRate >= RepeatThrottle then
		if C_Spell:IsCastAlmostDone() and C_Spell:IsActionAlmostReady(action) then
