...
function TargetFrame_OnLoad(self, unit, menuFunc)
	...
	self.background = _G[thisName.."Background"]
	...
	UnitFrame_Initialize(self, unit, _G[thisName.."TextureFrameName"], portraitFrame,
	                     ...
	                     threatFrame, "player", _G[thisName.."NumericalThreat"], nil, nil, nil, nil,
...
function TargetFrame_Update (self)
	...
	if ( not UnitExists(self.unit) ) then
		...
		if self.showPrimaryStat then
			TargetFrame_CheckPrimaryStat(self)
		end
...
function TargetFrame_OnEvent (self, event, ...)
	...
	if ( event == "PLAYER_ENTERING_WORLD" ) then
		RunNextFrame(function() TargetFrame_Update(self); end)
...
function TargetFrame_CheckFaction (self)
	if ( not UnitPlayerControlled(self.unit) and UnitIsTapped(self.unit) and not UnitIsTappedByPlayer(self.unit) and not UnitIsTappedByAllThreatList(self.unit) ) then
        ...
        if UnitIsFriend("player", self.unit) then
            if UnitIsPVPFreeForAll(self.unit) and not UnitIsPVPFreeForAll("player") then
                self.nameBackground:SetVertexColor(1, 1, 0);
            else
...
function TargetFrame_CheckClassification (self, forceNormalTexture)
	...
	if ( texture and not forceNormalTexture) then
		self.borderTexture:SetTexCoord(0.09375, 1, 0, 0.78125)
		self.levelText:SetPoint("CENTER", 63, -16)
		self.nameBackground:SetPoint("TOPRIGHT", -106, -22)
		self.healthbar:SetPoint("TOPRIGHT", -106, -41)
		self.manabar:SetPoint("TOPRIGHT", -106, -52)
...
function TargetFrame_CheckPrimaryStat(self)
	if not self.unit then
		if self.PrimaryStat:IsMouseOver() then
	...
	local stat = UnitPrimaryStat(self.unit)
	if stat and UnitIsPlayer(self.unit) then
		local spellID, _, icon = C_PrimaryStat:GetPrimaryStatInfo(stat)
		local tooltip = "spell:"..spellID
		self.PrimaryStat.tooltip = tooltip
		self.PrimaryStat.Icon:SetPortraitTexture(icon)
