...
[4] The Interface census is a superset of the code layer, and a loose file at
    the client root that beats the archive chain is reported with the bytes the
    CLIENT loads - each such record still carries a sha256.
[5] Determinism: content and cache re-emit byte-identically. (The Interface
    census is a by-product of the one traversal and cannot be re-emitted
    without it, so it is checked against the committed layer rather than
    re-run; its own emission cross-verifies every sha256 against the harvest,
    which is the stronger check anyway.)

MIGRATED, 2026-08. This drove `tools/extract_content.py`,
`tools/extract_cache.py`, `tools/extract_interface_all.py` and
`tools/inventory.py`, all retired by the single-script collapse, and it sat
failing on import for a commit. Every gate is the same; the inputs moved:

  * the emitters are `emit.emit_content` and `emit.emit_cache`, which take a
    SNAPSHOT directory and an output directory. That is what makes [5] a real
    determinism check again and a stricter one than before - the layer is
    rebuilt into a scratch tree and compared file by file to what is committed,
    instead of being rebuilt in place and hashed before and after.
  * "the files on disk" means the files in `work/snapshot/`, not in the live
    client. The layer was built from the snapshot; comparing it against a
    client the launcher has patched since would fail for a reason that is not a
    defect, which is the entire point of taking a snapshot.
  * the Interface path census comes from `raw/interface_all/paths/*.jsonl` and
    is cross-checked against `raw/_inventory`, rather than from a retired
    module's in-memory record list.
"""
import gzip
import hashlib
import json
...
IALL = config.RAW_DIR / "interface_all"
CACHE = config.RAW_DIR / "cache"
CONTENT = config.RAW_CONTENT_DIR
...
_iix = load(IALL / "index.json")
_stale = []
_checked = 0
for _fp in sorted((IALL / "paths").glob("*.jsonl")):
...
for _d in (CONTENT, CACHE, config.RAW_INTERFACE_DIR, IALL,
...
assert SNAP.is_dir(), (
    f"{SNAP} is missing - run `python datamine.py`. These gates compare the "
    f"layer against the SNAPSHOT it was built from, never against a live client "
    f"the launcher may have patched since.")

TMP = Path(tempfile.mkdtemp(prefix="coa_layers_"))
try:
    # ======================================================================
    # [1] content + localization
    # ======================================================================
    cix = emit.emit_content(SNAP, TMP)
    cix = load(TMP / "content" / "index.json")

    assert cix["failureCount"] == 0, cix["failures"]
    assert cix["unhandledCount"] == 0, "an unhandled file class appeared under Data\\Content"

    on_disk = sorted(p for p in (SNAP / "Data" / "Content").rglob("*")
                     if p.is_file())
    ...
    for g in cix["localizationGroups"]:
        d = CONTENT / "localization" / g["entity"] / g["locale"]
    ...
    spell_de = dict((r["id"], r["text"]) for _, r in read_shards(
        CONTENT / "localization" / "Spell/Name" / "deDE"))
    assert spell_de[17] == "Machtwort: Schild", spell_de.get(17)   # spell 17
    item_de = dict((r["id"], r["text"]) for _, r in read_shards(
        CONTENT / "localization" / "Item/Name" / "deDE"))
    assert item_de[17] == "Martinsfuror", item_de.get(17)          # item 17
    print(f"[1] content: {cix['fileCount']} files, {cix['locCount']} .loc closed "
          f"exactly, {cix['locRecordTotal']:,} records, 0 failures")

    # ======================================================================
    # [2] WDB caches - schemas earned, realms kept apart
    # ======================================================================
    emit.emit_cache(SNAP, TMP)
    kix = load(TMP / "cache" / "index.json")
    cache_on_disk = sorted(p for p in (SNAP / "Cache" / "WDB").rglob("*")
                           if p.is_file())
    ...
    for f in kix["failures"]:
        assert f["tier"] == "raw" and ("bytesBase64" in f
                                       or f.get("bytesCommitted") is False), f

    # the realm cache is its own group and is NOT the base one
    groups = list(kix["groups"])
    ...
    # same-kind same-width fields consumes identically), so names must not reach
    # the data. Checked over every schema-tier record, not a sample.
    ...
    interp = load(CACHE / "_interpretation.json")
    assert "INTERPRETATION" in interp["rule"].upper(), interp["rule"][:80]
    assert set(interp["magics"]) == set(wdb.SCHEMAS), sorted(interp["magics"])

    def at(magic: str, name: str) -> str:
        """The positional field the interpretation gives `name` for `magic`.
        Ground truth below is checked THROUGH this map, so it still catches a
        wrong layout while never requiring a name to appear in the data."""
    ...
    # the variable-length run: one positional slot holding a flat list, its
    # length equal to the declared count x the two kinds in the run
    ...
    for e in flat:
        ...
        assert all(k.startswith("f") for k in
                   next(r for _, r in read_shards(CACHE / e["dir"]))), e
    ...
    # ======================================================================
    iix = load(IALL / "index.json")

    # The census must cover exactly the Interface paths the file inventory
    # names. The inventory is built by a different pass over the same harvest,
    # so agreeing with it is agreeing with something this layer did not compute.
    # The inventory shards are named by path prefix, so Interface paths live in
    # the `interface-*` shards and nowhere else. That is a property of the
    # sharder, so it is CHECKED rather than assumed: any Interface path found
    # in another shard would break the subset test below by being absent from
    # `inv_paths`, and the whole-inventory scan is 166 MB, which is not worth
    # paying on every run to re-derive a rule the sharder already publishes.
    inv_dir = config.RAW_DIR / "_inventory" / "files"
    inv_paths = set()
    for fp in sorted(inv_dir.glob("interface*.json")):
        for r in load(fp):
            if r["path"].lower().startswith("interface/"):
                inv_paths.add(r["path"])
    assert len(inv_paths) > 10000, len(inv_paths)

    seen = {}
    for fp in sorted((IALL / "paths").glob("*.jsonl")):
        ...
        for r in rows:
            ...
            seen[r["path"]] = r
    assert len(seen) == iix["pathCount"], (len(seen), iix["pathCount"])
    assert inv_paths <= set(seen), sorted(inv_paths - set(seen))[:5]

    # a loose file at the client root beats every archive that carries the path,
    # and the census must report the bytes the CLIENT loads, not the archive's
    loose_recs = [r for r in seen.values() if r.get("source") == "loose"]
    for r in loose_recs:
        disk = SNAP / r["diskPath"].replace("/", "\\")
    ...
    assert len(loose_recs) == len(iix["looseOverrides"]), \
        (len(loose_recs), len(iix["looseOverrides"]))
    print(f"      {len(loose_recs)} Interface paths won by a loose file on disk, "
          f"each hashed against the snapshot")
    ...
    code = load(config.RAW_INTERFACE_DIR / "_manifest.json")
    arch_code = {("Interface/" + k.replace("\\", "/"))
                 for k, m in code["files"].items() if m["source"] != "disk"}
    assert arch_code <= set(seen), sorted(arch_code - set(seen))[:5]

    # The scope boundary is a RULE, not a live count of the user's addons. The
    # numbers used to be here (fileCount / bytes / addonDirCount / the names of
    # all 145 addon directories) and they broke byte-for-byte reproduction on an
    # unchanged client the moment the user installed anything: two independent
    # runs of this pipeline over a bit-identical snapshot differed in exactly
    # two files, and this was one of them.
    boundary = iix["installStateBoundary"]
    assert boundary["excluded"] and boundary["rule"] and boundary["exception"]
    leaked = [k for k in boundary
              if k in ("fileCount", "bytes", "addonDirs", "addonDirCount")]
    assert not leaked, f"install state committed into the layer again: {leaked}"
    print(f"[3+4] interface: {iix['pathCount']:,} paths ({iix['textCount']:,} text "
          f"bytes committed, {iix['binaryCount']:,} binary hashed), superset of "
          f"the {len(arch_code)} archive-sourced code files, and the scope "
          f"boundary carries no install-specific number")

    # ======================================================================
    # [5] determinism
    # ======================================================================
    for layer, src in (("content", CONTENT), ("cache", CACHE)):
        rebuilt = digest_tree(TMP / layer)
        live = digest_tree(src)
        changed = sorted(k for k in set(rebuilt) | set(live)
                         if rebuilt.get(k) != live.get(k))
        assert not changed, (f"re-emitting {layer} changed {len(changed)} files: "
                             f"{changed[:5]}")
        assert len(rebuilt) > 10, (layer, len(rebuilt))
        print(f"[5] determinism: {layer} re-emitted from the snapshot into a "
              f"scratch tree - {len(rebuilt)} files byte-identical to what is "
              f"committed")
finally:
